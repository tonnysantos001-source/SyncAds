...
-- Versão simplificada para aplicação direta
...
CREATE POLICY "Users view own devices" ON "ExtensionDevice" FOR SELECT USING (auth.uid() = "userId");

DROP POLICY IF EXISTS "Users insert own devices" ON "ExtensionDevice";
CREATE POLICY "Users insert own devices" ON "ExtensionDevice" FOR INSERT WITH CHECK (auth.uid() = "userId");

DROP POLICY IF EXISTS "Users update own devices" ON "ExtensionDevice";
CREATE POLICY "Users update own devices" ON "ExtensionDevice" FOR UPDATE USING (auth.uid() = "userId");

-- Policies ExtensionCommand
DROP POLICY IF EXISTS "Users view own commands" ON "ExtensionCommand";
CREATE POLICY "Users view own commands" ON "ExtensionCommand" FOR SELECT USING (auth.uid() = "userId");

DROP POLICY IF EXISTS "Users insert own commands" ON "ExtensionCommand";
CREATE POLICY "Users insert own commands" ON "ExtensionCommand" FOR INSERT WITH CHECK (auth.uid() = "userId");

DROP POLICY IF EXISTS "Users update own commands" ON "ExtensionCommand";
CREATE POLICY "Users update own commands" ON "ExtensionCommand" FOR UPDATE USING (auth.uid() = "userId");

-- Policies ExtensionLog
DROP POLICY IF EXISTS "Users view own logs" ON "ExtensionLog";
CREATE POLICY "Users view own logs" ON "ExtensionLog" FOR SELECT USING (auth.uid() = "userId");

DROP POLICY IF EXISTS "Users insert own logs" ON "ExtensionLog";
CREATE POLICY "Users insert own logs" ON "ExtensionLog" FOR INSERT WITH CHECK (auth.uid() = "userId");
