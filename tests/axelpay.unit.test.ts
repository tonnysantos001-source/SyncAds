...
import { Service } from "../supabase/functions/integrations/domain/payment/providers/axelpay/v1/service.ts";
import { Client } from "../supabase/functions/integrations/domain/payment/providers/axelpay/v1/client.ts";
import { WebhookHandler } from "../supabase/functions/integrations/domain/payment/providers/axelpay/v1/webhook.ts";

describe("Axelpay Provider - Unit Tests", () => {
  describe("Validator", () => {
    it("should fail validation if storeId is missing", () => {
      const result = Validator.validateCredentials({ token: "tok_123" });
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("Store ID (storeId) é obrigatório para o Axelpay.");
    });

    it("should fail validation if token is missing", () => {
      const result = Validator.validateCredentials({ storeId: "store_123" });
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("Token de API (token) é obrigatório para o Axelpay.");
    });
  ...
  describe("Mapper", () => {
    ...
    it("should map response correctly for PIX", () => {
      ...
      expect(result.transactionId).toBe("pay_axel_999");
  ...
  describe("Service", () => {
    ...
    it("should process PIX payment successfully", async () => {
      mockHttp.request.mockResolvedValueOnce({
        ...
        status: 200,
        json: async () => ({
          transaction_id: "tx_axel_01",
          status: "pending",
          qr_code: "qr_code_content_axel",
        }),
      });
      ...
      expect(result.success).toBe(true);
      expect(result.transactionId).toBe("tx_axel_01");
      expect(result.qrCode).toBe("qr_code_content_axel");
    });
  });
});
