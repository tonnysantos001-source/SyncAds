...
import { Service } from "../supabase/functions/integrations/domain/payment/providers/flowspay/v1/service.ts";
import { Client } from "../supabase/functions/integrations/domain/payment/providers/flowspay/v1/client.ts";
import { WebhookHandler } from "../supabase/functions/integrations/domain/payment/providers/flowspay/v1/webhook.ts";

describe("Flowspay Provider - Unit Tests", () => {
  describe("Validator", () => {
    it("should fail validation if clientId is missing", () => {
      const result = Validator.validateCredentials({ clientSecret: "sec_123" });
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("Client ID (clientId) é obrigatório para o Flowspay.");
    });

    it("should fail validation if clientSecret is missing", () => {
      const result = Validator.validateCredentials({ clientId: "cli_123" });
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("Client Secret (clientSecret) é obrigatório para o Flowspay.");
    });
  ...
  describe("Mapper", () => {
    ...
    it("should map response correctly for PIX", () => {
      ...
      expect(result.transactionId).toBe("pay_flow_999");
  ...
  describe("Service", () => {
    ...
    it("should process PIX payment successfully", async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.transactionId).toBe("tx_flow_01");
      expect(result.qrCode).toBe("qr_code_content_flow");
    });
  });
});
