...
import { Service } from "../supabase/functions/integrations/domain/payment/providers/blackcat/v1/service.ts";
import { Client } from "../supabase/functions/integrations/domain/payment/providers/blackcat/v1/client.ts";

describe("Blackcat Provider - Unit Tests", () => {
  describe("Validator", () => {
    it("should fail validation if apiKey is missing", () => {
      const result = Validator.validateCredentials({ secretKey: "abc" });
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("A apiKey é obrigatória.");
    });

    it("should fail validation if secretKey is missing", () => {
      const result = Validator.validateCredentials({ apiKey: "abc" });
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("A secretKey é obrigatória.");
    });

    it("should pass validation with both apiKey and secretKey present", () => {
      const result = Validator.validateCredentials({ apiKey: "abc", secretKey: "def" });
  ...
  describe("Service Integration & Execution", () => {
    ...
    it("should validate credentials via ping successfully", async () => {
      ...
      const res = await service.validateCredentials({ apiKey: "valid_key", secretKey: "sec" });
      expect(res.isValid).toBe(true);
      expect(res.message).toBe("Conexão com Blackcat validada com sucesso.");
    });
    ...
    it("should process Pix creation successfully", async () => {
      ...
      const config: any = {
        credentials: { apiKey: "test_key", secretKey: "sec" },
        isTestMode: true,
      };
