...
const validCreds = { apiKey: "dp_live_test_key_xyz_777", secretKey: "sec_key_xyz_777" };
...
describe("Dorapag Mapper", () => {
  it("converte amount para centavos", () => {
    const p = Mapper.toCreatePaymentPayload(validRequest);
    expect(p.amount).toBe(15000);
  });
  it("mapeia status Dorapag: approved → approved", () => expect(Mapper.toPaymentStatus("approved")).toBe("approved"));
  it("mapeia status Dorapag: failed → failed", () => expect(Mapper.toPaymentStatus("failed")).toBe("failed"));
...
describe("Dorapag WebhookHandler", () => {
