...
import { Validator } from "../supabase/functions/integrations/domain/payment/providers/ame/v1/validator.ts";
import { Mapper } from "../supabase/functions/integrations/domain/payment/providers/ame/v1/mapper.ts";
import { WebhookHandler } from "../supabase/functions/integrations/domain/payment/providers/ame/v1/webhook.ts";

const validCreds = { clientId: "test_clientId_value", clientSecret: "test_clientSecret_value" };
const validRequest: any = {
  orderId: "ORDER-AME-001",
  amount: 150.00,
  paymentMethod: "pix",
  customer: { name: "Maria Santos", email: "maria@example.com", document: "12345678909" }
};

describe("Ame Digital Validator", () => {
...
describe("Ame Digital Mapper", () => {
  ...
  it("mapeia resposta de criacao de pagamento", () => {
    ...
    const r = Mapper.toPaymentResponse(api, "ORDER-AME-001");
...
describe("Ame Digital WebhookHandler", () => {
  it("processa webhook de aprovacao", () => {
    const payload = { transaction: { orderId: "ORDER-AME-001", status: "approved" } };
    const r = WebhookHandler.handle(payload);
    expect(r.success).toBe(true);
    expect(r.transactionId).toBe("ORDER-AME-001");
  });
});
