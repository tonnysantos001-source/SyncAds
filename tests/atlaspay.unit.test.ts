...
import { Service } from "../supabase/functions/integrations/domain/payment/providers/atlaspay/v1/service.ts";
import { Client } from "../supabase/functions/integrations/domain/payment/providers/atlaspay/v1/client.ts";
import { WebhookHandler } from "../supabase/functions/integrations/domain/payment/providers/atlaspay/v1/webhook.ts";

describe("Atlas Pay Provider - Unit Tests", () => {
  describe("Validator", () => {
    it("should fail validation if apiId is missing", () => {
      const result = Validator.validateCredentials({ apiSecret: "sec_123" });
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("API ID (apiId) é obrigatório para o Atlas Pay.");
    });

    it("should fail validation if apiSecret is missing", () => {
      const result = Validator.validateCredentials({ apiId: "id_123" });
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("API Secret (apiSecret) é obrigatório para o Atlas Pay.");
    });

    it("should pass validation with valid credentials", () => {
      const result = Validator.validateCredentials({ apiId: "id_123", apiSecret: "sec_123" });
  ...
  describe("Mapper", () => {
    ...
    it("should map response correctly for PIX", () => {
      ...
      expect(result.transactionId).toBe("pay_atlas_999");
  ...
  describe("Service", () => {
    ...
    it("should process PIX payment successfully", async () => {
      mockHttp.request.mockResolvedValueOnce({
        ...
        status: 200,
        json: async () => ({
          transaction_id: "tx_atlas_01",
          status: "pending",
          qr_code: "qr_code_content_atlas",
        }),
      });
      ...
      const integrationConfig: any = {
        credentials: { apiId: "id_123", apiSecret: "sec_123" },
        isTestMode: true,
      };

      const result = await service.createPix(request, integrationConfig);
      expect(result.success).toBe(true);
      expect(result.transactionId).toBe("tx_atlas_01");
      expect(result.qrCode).toBe("qr_code_content_atlas");
    });
  });
});
