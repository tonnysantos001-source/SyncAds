...
import { Validator } from "../supabase/functions/integrations/domain/payment/providers/fivepay/v1/validator.ts";
import { Mapper } from "../supabase/functions/integrations/domain/payment/providers/fivepay/v1/mapper.ts";
import { Service } from "../supabase/functions/integrations/domain/payment/providers/fivepay/v1/service.ts";
import { Client } from "../supabase/functions/integrations/domain/payment/providers/fivepay/v1/client.ts";
import { WebhookHandler } from "../supabase/functions/integrations/domain/payment/providers/fivepay/v1/webhook.ts";

describe("Fivepay Provider - Unit Tests", () => {
  describe("Validator", () => {
    it("should fail validation if partnerId is missing", () => {
      const result = Validator.validateCredentials({ apiKey: "key_123" });
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("Partner ID (partnerId) é obrigatório para o Fivepay.");
    });

    it("should fail validation if apiKey is missing", () => {
      const result = Validator.validateCredentials({ partnerId: "part_123" });
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("Chave de API (apiKey) é obrigatória para o Fivepay.");
    });
  ...
  describe("Mapper", () => {
    it("should map request to payment payload correctly", () => {
      const request: any = {
        amount: 50.00,
        orderId: "ord_fivepay_01",
        paymentMethod: "pix",
        customer: {
          name: "Fivepay Customer",
          email: "fivepay@fivepay.com",
          document: "123.456.789-00",
        },
      };

      const payload = Mapper.toPaymentPayload(request);
      expect(payload.transaction_id).toBe("ord_fivepay_01");
      expect(payload.amount).toBe(50.00);
    });

    it("should map response correctly for PIX", () => {
      const mockResponse: any = {
        transaction_id: "pay_fivepay_999",
        status: "success",
      ...
      expect(result.transactionId).toBe("pay_fivepay_999");
  ...
  describe("Service", () => {
    ...
    it("should process PIX payment successfully", async () => {
      mockHttp.request.mockResolvedValueOnce({
        ...
        status: 200,
        json: async () => ({
          transaction_id: "tx_fivepay_01",
          status: "pending",
          qr_code: "qr_code_content_fivepay",
        }),
      });
      ...
      expect(result.success).toBe(true);
      expect(result.transactionId).toBe("tx_fivepay_01");
      expect(result.qrCode).toBe("qr_code_content_fivepay");
    });
  });
});
