...
const validCreds = { apiKey: "fortrex_live_test_key_xyz_777", merchantCode: "merchant_12345" };
const validRequest: any = {
  orderId: "ORDER-FT-001", amount: 150.00, paymentMethod: "credit_card", installments: 1,
...
describe("Fortrex Validator", () => {
  ...
  it("rejeita merchantCode vazia", () => expect(Validator.validateCredentials({ ...validCreds, merchantCode: "" }).isValid).toBe(false));
...
describe("Fortrex Mapper", () => {
  it("converte amount para centavos", () => {
    const p = Mapper.toCreatePaymentPayload(validRequest);
    expect(p.amount).toBe(15000);
  });
  it("mapeia status Fortrex: approved → approved", () => expect(Mapper.toPaymentStatus("approved")).toBe("approved"));
  it("mapeia status Fortrex: failed → failed", () => expect(Mapper.toPaymentStatus("failed")).toBe("failed"));
  it("mapeia resposta de criação com sucesso", () => {
    const api = { id: "ch_555", transaction_id: "ORDER-FT-001", status: "approved", amount: 15000 };
    const r = Mapper.toPaymentResponse(api as any, "ORDER-FT-001");
...
describe("Fortrex WebhookHandler", () => {
  it("processa webhook de transação paga", () => {
    const payload = { id: "ch_555", status: "approved", transaction_id: "ORDER-FT-001" };
    const r = WebhookHandler.handle(payload);
    expect(r.success).toBe(true);
    expect(r.status).toBe("approved");
    expect(r.transactionId).toBe("ORDER-FT-001");
  });
