...
import { Validator } from "../supabase/functions/integrations/domain/payment/providers/fitbank/v1/validator.ts";
import { Mapper } from "../supabase/functions/integrations/domain/payment/providers/fitbank/v1/mapper.ts";
import { WebhookHandler } from "../supabase/functions/integrations/domain/payment/providers/fitbank/v1/webhook.ts";

const validCreds = { businessUnit: "test_businessUnit_value", patternId: "test_patternId_value", password: "test_password_value" };
const validRequest: any = {
  orderId: "ORDER-FITBANK-001",
  amount: 150.00,
  paymentMethod: "pix",
  customer: { name: "Maria Santos", email: "maria@example.com", document: "12345678909" }
};

describe("FitBank Validator", () => {
  it("aceita credenciais válidas", () => expect(Validator.validateCredentials(validCreds).isValid).toBe(true));
  it("rejeita credenciais inválidas", () => expect(Validator.validateCredentials({ businessUnit: "", patternId: "val", password: "val" }).isValid).toBe(false));
...
describe("FitBank Mapper", () => {
  ...
  it("mapeia resposta de criacao de pagamento", () => {
    ...
    const r = Mapper.toPaymentResponse(api, "ORDER-FITBANK-001");
...
describe("FitBank WebhookHandler", () => {
  it("processa webhook de aprovacao", () => {
    const payload = { transaction: { orderId: "ORDER-FITBANK-001", status: "approved" } };
    const r = WebhookHandler.handle(payload);
    expect(r.success).toBe(true);
    expect(r.transactionId).toBe("ORDER-FITBANK-001");
  });
});
