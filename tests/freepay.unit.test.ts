...
const validCreds = { token: "freepay_live_test_key_xyz_777", accountId: "acc_12345" };
...
describe("Freepay Mapper", () => {
  it("converte amount para centavos", () => {
    const p = Mapper.toCreatePaymentPayload(validRequest);
    expect(p.amount).toBe(15000);
  });
  it("mapeia status Freepay: approved → approved", () => expect(Mapper.toPaymentStatus("approved")).toBe("approved"));
  it("mapeia status Freepay: failed → failed", () => expect(Mapper.toPaymentStatus("failed")).toBe("failed"));
...
describe("Freepay WebhookHandler", () => {
