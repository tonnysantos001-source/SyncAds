...
import { Validator } from "../supabase/functions/integrations/domain/payment/providers/sipag/v1/validator.ts";
import { Mapper } from "../supabase/functions/integrations/domain/payment/providers/sipag/v1/mapper.ts";
import { WebhookHandler } from "../supabase/functions/integrations/domain/payment/providers/sipag/v1/webhook.ts";

const validCreds = { merchantId: "test_merchantId_value", secretKey: "test_secretKey_value" };
const validRequest: any = {
  orderId: "ORDER-SIPAG-001",
  amount: 150.00,
  paymentMethod: "pix",
  customer: { name: "Maria Santos", email: "maria@example.com", document: "12345678909" }
};

describe("Sipag Validator", () => {
  it("aceita credenciais válidas", () => expect(Validator.validateCredentials(validCreds).isValid).toBe(true));
  it("rejeita credenciais inválidas", () => expect(Validator.validateCredentials({ merchantId: "", secretKey: "val" }).isValid).toBe(false));
...
describe("Sipag Mapper", () => {
  ...
  it("mapeia resposta de criacao de pagamento", () => {
    ...
    const r = Mapper.toPaymentResponse(api, "ORDER-SIPAG-001");
...
describe("Sipag WebhookHandler", () => {
  it("processa webhook de aprovacao", () => {
    const payload = { transaction: { orderId: "ORDER-SIPAG-001", status: "approved" } };
    const r = WebhookHandler.handle(payload);
    expect(r.success).toBe(true);
    expect(r.transactionId).toBe("ORDER-SIPAG-001");
  });
});
