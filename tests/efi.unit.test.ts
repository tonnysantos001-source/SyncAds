...
import { WebhookHandler } from "../supabase/functions/integrations/domain/payment/providers/efi/v1/webhook.ts";

const validCreds = { clientId: "efi_client_id_xyz", clientSecret: "sec_12345" };
const validRequest: any = {
  orderId: "ORDER-EFI-001", amount: 150.00, paymentMethod: "credit_card", installments: 1,
...
describe("Efí Mapper", () => {
  ...
  it("mapeia status Efí: approved → approved", () => expect(Mapper.toPaymentStatus("approved")).toBe("approved"));
  it("mapeia status Efí: failed → failed", () => expect(Mapper.toPaymentStatus("failed")).toBe("failed"));
  it("mapeia resposta de criação com sucesso", () => {
    const api = { id: "ch_555", transaction_id: "ORDER-EFI-001", status: "approved", amount: 15000 };
    const r = Mapper.toPaymentResponse(api as any, "ORDER-EFI-001");
...
describe("Efí WebhookHandler", () => {
  it("processa webhook de transação paga", () => {
    const payload = { id: "ch_555", status: "approved", transaction_id: "ORDER-EFI-001" };
    const r = WebhookHandler.handle(payload);
    expect(r.success).toBe(true);
    expect(r.status).toBe("approved");
    expect(r.transactionId).toBe("ORDER-EFI-001");
  });
