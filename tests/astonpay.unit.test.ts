...
import { Service } from "../supabase/functions/integrations/domain/payment/providers/astonpay/v1/service.ts";
import { Client } from "../supabase/functions/integrations/domain/payment/providers/astonpay/v1/client.ts";
import { WebhookHandler } from "../supabase/functions/integrations/domain/payment/providers/astonpay/v1/webhook.ts";

describe("Aston Pay Provider - Unit Tests", () => {
  describe("Validator", () => {
    it("should fail validation if merchantCode is missing", () => {
      const result = Validator.validateCredentials({ apiKey: "key_123" });
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("Merchant Code (merchantCode) é obrigatório para o Aston Pay.");
    });

    it("should fail validation if apiKey is missing", () => {
      const result = Validator.validateCredentials({ merchantCode: "code_123" });
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("API Key (apiKey) é obrigatória para o Aston Pay.");
    });

    it("should pass validation with valid credentials", () => {
      const result = Validator.validateCredentials({ merchantCode: "code_123", apiKey: "key_123" });
  ...
  describe("Mapper", () => {
    ...
    it("should map response correctly for PIX", () => {
      ...
      expect(result.transactionId).toBe("pay_aston_999");
  ...
  describe("Service", () => {
    ...
    it("should process PIX payment successfully", async () => {
      ...
      const integrationConfig: any = {
        credentials: { merchantCode: "code_123", apiKey: "key_123" },
        isTestMode: true,
      };

      const result = await service.createPix(request, integrationConfig);
      expect(result.success).toBe(true);
      expect(result.transactionId).toBe("tx_aston_01");
