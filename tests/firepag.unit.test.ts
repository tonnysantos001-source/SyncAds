...
const validCreds = { token: "fp_live_test_key_xyz_777", storeId: "store_12345" };
const validRequest: any = {
  orderId: "ORDER-FP-001", amount: 150.00, paymentMethod: "credit_card", installments: 1,
...
describe("Firepag Validator", () => {
  ...
  it("rejeita storeId vazia", () => expect(Validator.validateCredentials({ ...validCreds, storeId: "" }).isValid).toBe(false));
...
describe("Firepag Mapper", () => {
  it("converte amount para centavos", () => {
    const p = Mapper.toCreatePaymentPayload(validRequest);
    expect(p.amount).toBe(15000);
  });
  it("mapeia status Firepag: approved → approved", () => expect(Mapper.toPaymentStatus("approved")).toBe("approved"));
  it("mapeia status Firepag: failed → failed", () => expect(Mapper.toPaymentStatus("failed")).toBe("failed"));
  it("mapeia resposta de criação com sucesso", () => {
    const api = { id: "ch_555", transaction_id: "ORDER-FP-001", status: "approved", amount: 15000 };
    const r = Mapper.toPaymentResponse(api as any, "ORDER-FP-001");
...
describe("Firepag WebhookHandler", () => {
  it("processa webhook de transação paga", () => {
    const payload = { id: "ch_555", status: "approved", transaction_id: "ORDER-FP-001" };
    const r = WebhookHandler.handle(payload);
    expect(r.success).toBe(true);
    expect(r.status).toBe("approved");
    expect(r.transactionId).toBe("ORDER-FP-001");
  });
