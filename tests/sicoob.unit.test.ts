...
import { Validator } from "../supabase/functions/integrations/domain/payment/providers/sicoob/v1/validator.ts";
import { Mapper } from "../supabase/functions/integrations/domain/payment/providers/sicoob/v1/mapper.ts";
import { WebhookHandler } from "../supabase/functions/integrations/domain/payment/providers/sicoob/v1/webhook.ts";

const validCreds = { clientId: "test_clientId_value", clientSecret: "test_clientSecret_value" };
const validRequest: any = {
  orderId: "ORDER-SICOOB-001",
  amount: 150.00,
  paymentMethod: "pix",
  customer: { name: "Maria Santos", email: "maria@example.com", document: "12345678909" }
};

describe("Sicoob API Validator", () => {
...
describe("Sicoob API Mapper", () => {
  ...
  it("mapeia resposta de criacao de pagamento", () => {
    ...
    const r = Mapper.toPaymentResponse(api, "ORDER-SICOOB-001");
...
describe("Sicoob API WebhookHandler", () => {
  it("processa webhook de aprovacao", () => {
    const payload = { transaction: { orderId: "ORDER-SICOOB-001", status: "approved" } };
    const r = WebhookHandler.handle(payload);
    expect(r.success).toBe(true);
    expect(r.transactionId).toBe("ORDER-SICOOB-001");
  });
});
