...
import { Service } from "../supabase/functions/integrations/domain/payment/providers/axionpay/v1/service.ts";
import { Client } from "../supabase/functions/integrations/domain/payment/providers/axionpay/v1/client.ts";
import { WebhookHandler } from "../supabase/functions/integrations/domain/payment/providers/axionpay/v1/webhook.ts";

describe("Axion Pay Provider - Unit Tests", () => {
  describe("Validator", () => {
    it("should fail validation if clientId is missing", () => {
      const result = Validator.validateCredentials({ clientSecret: "sec_123" });
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("Client ID (clientId) é obrigatório para o Axion Pay.");
    });

    it("should fail validation if clientSecret is missing", () => {
      const result = Validator.validateCredentials({ clientId: "id_123" });
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("Client Secret (clientSecret) é obrigatório para o Axion Pay.");
    });
  ...
  describe("Mapper", () => {
    ...
    it("should map response correctly for PIX", () => {
      ...
      expect(result.transactionId).toBe("pay_axion_999");
  ...
  describe("Service", () => {
    ...
    it("should process PIX payment successfully", async () => {
      mockHttp.request.mockResolvedValueOnce({
        ...
        status: 200,
        json: async () => ({
          transaction_id: "tx_axion_01",
          status: "pending",
          qr_code: "qr_code_content_axion",
        }),
      });
      ...
      const integrationConfig: any = {
        credentials: { clientId: "id_123", clientSecret: "sec_123" },
        isTestMode: true,
      };

      const result = await service.createPix(request, integrationConfig);
      expect(result.success).toBe(true);
      expect(result.transactionId).toBe("tx_axion_01");
      expect(result.qrCode).toBe("qr_code_content_axion");
    });
  });
});
