...
const validCreds = { token: "dom_live_test_key_xyz_777", merchantId: "merch_12345" };
...
describe("Dom Pagamentos Validator", () => {
...
describe("Dom Pagamentos Mapper", () => {
  it("converte amount para centavos", () => {
    const p = Mapper.toCreatePaymentPayload(validRequest);
    expect(p.amount).toBe(15000);
  });
  it("mapeia status Dom Pagamentos: approved → approved", () => expect(Mapper.toPaymentStatus("approved")).toBe("approved"));
  it("mapeia status Dom Pagamentos: failed → failed", () => expect(Mapper.toPaymentStatus("failed")).toBe("failed"));
...
describe("Dom Pagamentos WebhookHandler", () => {
