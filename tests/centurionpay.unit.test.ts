...
import { Service } from "../supabase/functions/integrations/domain/payment/providers/centurionpay/v1/service.ts";
import { Client } from "../supabase/functions/integrations/domain/payment/providers/centurionpay/v1/client.ts";
import { WebhookHandler } from "../supabase/functions/integrations/domain/payment/providers/centurionpay/v1/webhook.ts";

describe("Centurion Pay Provider - Unit Tests", () => {
  describe("Validator", () => {
    it("should fail validation if merchantId is missing", () => {
      const result = Validator.validateCredentials({ apiToken: "tok_123" });
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("Merchant ID (merchantId) é obrigatório para o Centurion Pay.");
    });

    it("should fail validation if apiToken is missing", () => {
      const result = Validator.validateCredentials({ merchantId: "merch_123" });
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("Token de API (apiToken) é obrigatório para o Centurion Pay.");
    });
  ...
  describe("Mapper", () => {
    it("should map request to payment payload correctly", () => {
      const request: any = {
        amount: 50.00,
        orderId: "ord_centurion_01",
        paymentMethod: "pix",
        customer: {
          name: "Centurion Customer",
          email: "centurion@centurion.com",
          document: "123.456.789-00",
        },
      };

      const payload = Mapper.toPaymentPayload(request);
      expect(payload.transaction_id).toBe("ord_centurion_01");
      expect(payload.amount).toBe(50.00);
    });

    it("should map response correctly for PIX", () => {
      const mockResponse: any = {
        transaction_id: "pay_centurion_999",
        status: "success",
      ...
      expect(result.transactionId).toBe("pay_centurion_999");
  ...
  describe("Service", () => {
    ...
    it("should process PIX payment successfully", async () => {
      mockHttp.request.mockResolvedValueOnce({
        ...
        status: 200,
        json: async () => ({
          transaction_id: "tx_centurion_01",
          status: "pending",
          qr_code: "qr_code_content_centurion",
        }),
      });
      ...
      expect(result.success).toBe(true);
      expect(result.transactionId).toBe("tx_centurion_01");
      expect(result.qrCode).toBe("qr_code_content_centurion");
    });
  });
});
