...
import { Validator } from "../supabase/functions/integrations/domain/payment/providers/perfectmoney/v1/validator.ts";
import { Mapper } from "../supabase/functions/integrations/domain/payment/providers/perfectmoney/v1/mapper.ts";
import { WebhookHandler } from "../supabase/functions/integrations/domain/payment/providers/perfectmoney/v1/webhook.ts";

const validCreds = { memberId: "test_memberId_value", alternatePassphrase: "test_alternatePassphrase_value" };
const validRequest: any = {
  orderId: "ORDER-PERFECTMONEY-001",
  amount: 150.00,
  paymentMethod: "pix",
  customer: { name: "Maria Santos", email: "maria@example.com", document: "12345678909" }
};

describe("Perfect Money Validator", () => {
  it("aceita credenciais válidas", () => expect(Validator.validateCredentials(validCreds).isValid).toBe(true));
  it("rejeita credenciais inválidas", () => expect(Validator.validateCredentials({ memberId: "", alternatePassphrase: "val" }).isValid).toBe(false));
...
describe("Perfect Money Mapper", () => {
  ...
  it("mapeia resposta de criacao de pagamento", () => {
    ...
    const r = Mapper.toPaymentResponse(api, "ORDER-PERFECTMONEY-001");
...
describe("Perfect Money WebhookHandler", () => {
  it("processa webhook de aprovacao", () => {
    const payload = { transaction: { orderId: "ORDER-PERFECTMONEY-001", status: "approved" } };
    const r = WebhookHandler.handle(payload);
    expect(r.success).toBe(true);
    expect(r.transactionId).toBe("ORDER-PERFECTMONEY-001");
  });
});
