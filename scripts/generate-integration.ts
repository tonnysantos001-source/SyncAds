// =========================================================================
// SCRIPT CLI - GERADOR DE MÓDULOS DE INTEGRAÇÃO (Deno)
// Execução: deno run -A scripts/generate-integration.ts --slug=<slug> --name="<Name>" --category=<payment|logistics|crm...> --version=<v1|v2...>
// =========================================================================
...
const args = parse(Deno.args, {
  string: ["slug", "name", "category", "version"],
  default: {
    version: "v1",
    category: "payment",
  },
...
const { slug, name, category, version } = args;

if (!slug || !name) {
  console.error("❌ Erro: --slug e --name são parâmetros obrigatórios!");
  console.log('Exemplo: deno run -A scripts/generate-integration.ts --slug=mercadopago --name="Mercado Pago" --category=payment --version=v1');
  Deno.exit(1);
}

const baseDir = join(Deno.cwd(), "supabase", "functions", "integrations", "domain", category, "providers", slug, version);
...
await ensureDir(join(baseDir, "fixtures"));
...
`;
await Deno.writeTextFile(join(baseDir, "config.ts"), configContent);
...
`;
await Deno.writeTextFile(join(baseDir, "types.ts"), typesContent);
...
`;
await Deno.writeTextFile(join(baseDir, "client.ts"), clientContent);
...
`;
await Deno.writeTextFile(join(baseDir, "validator.ts"), validatorContent);

// 6. Escrever mapper.ts
const mapperContent = `import { PaymentRequest, PaymentResponse, PaymentStatus, PaymentMethod } from "../../../../types.ts";
...
`;
await Deno.writeTextFile(join(baseDir, "mapper.ts"), mapperContent);
...
`;
await Deno.writeTextFile(join(baseDir, "webhook.ts"), webhookContent);
...
`;
await Deno.writeTextFile(join(baseDir, "service.ts"), serviceContent);

// 9. Escrever README.md & CHANGELOG.md
await Deno.writeTextFile(join(baseDir, "README.md"), `# Integração com ${name} (${slug}@${version})\n\nDocumentação oficial e fluxos do adaptador.`);
await Deno.writeTextFile(join(baseDir, "CHANGELOG.md"), `# Changelog - ${name}\n\n- **${new Date().toISOString().split('T')[0]}**: Inicialização do boilerplate da integração.`);

// 10. Escrever fixtures e templates
await Deno.writeTextFile(join(baseDir, "fixtures", "payment_created.json"), JSON.stringify({ id: "ch_123456", status: "pending", amount: 1000 }, null, 2));
await Deno.writeTextFile(join(baseDir, "fixtures", "payment_approved.json"), JSON.stringify({ id: "ch_123456", status: "approved", amount: 1000 }, null, 2));
await Deno.writeTextFile(join(baseDir, "sandbox.json"), JSON.stringify({ apiKey: "sk_test_mock_keys" }, null, 2));
...
await Deno.writeTextFile(join(baseDir, "plugin.json"), JSON.stringify(pluginJson, null, 2));
...
`;
await Deno.writeTextFile(join(baseDir, "tests", "unit.test.ts"), unitTestContent);
...
`;
await Deno.writeTextFile(join(baseDir, "tests", "integration.test.ts"), integrationTestContent);

console.log("✅ Geração concluída com sucesso!");
console.log(`💡 Para testar o unitário gerado execute: deno test --allow-read ${join(baseDir, "tests", "unit.test.ts")}`);
