...
const PYTHON_SERVICE_URL = process.env.VITE_PYTHON_SERVICE_URL || 'http://localhost:8000';
 ...
 */
export async function htmlToPDF(html: string, css?: string): Promise<PDFResult> {
...
export async function applyImageFilter(
  imageBase64: string,
  filterType: 'blur' | 'sharpen' | 'grayscale' | 'sepia' | 'vintage' | 'contrast' | 'brightness',
 ...
 */
export async function removeBackground(imageBase64: string): Promise<ImageResult> {
  return callPythonService<ImageResult>('/api/images/remove-background', 'POST', {
    image_base64: imageBase64,
  });
}
