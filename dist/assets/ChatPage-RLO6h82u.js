var ka=Object.defineProperty;var Pa=(s,e,t)=>e in s?ka(s,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):s[e]=t;var we=(s,e,t)=>Pa(s,typeof e!="symbol"?e+"":e,t);import{c as ke,r as d,R as Ce,o as Ma,p as _a,q as La,j as a,t as qa,v as sa,w as $a,x as za,y as Ga,z as ra,A as Fa,P as Ua,D as ee,E as Va,F as Ba,G as Ha,H as Wa,J as Qa,K as Ka,M as pe,S as oa,N as re,g as Q,l as K,B as k,s as D,n as se,h as Xa,i as Ya,O as Za,u as Be,e as Ja,Q as U,T as et,f as at,X as He,U as tt,a as ye,V as st}from"./index-CYU-cag4.js";import{G as na,S as rt}from"./send-D6LLUTVQ.js";import{E as oe}from"./external-link-BVkSvm4O.js";import{C as ot}from"./clock-C9BfP3rR.js";import{D as je}from"./download-CCiZYorJ.js";import{P as nt}from"./progress-BhXn__hj.js";import{C as it,A as ct}from"./AiThinkingIndicator-fXmqqQ2D.js";import{C as Te}from"./circle-alert-B3ytw7rB.js";import{C as Pe}from"./circle-x-DFN0MB7l.js";import{C as lt}from"./circle-check-CGfq9sJZ.js";import{i as ue,I as Ne}from"./integrationsService-9ZiHajz9.js";import{P as dt}from"./plus-Co35UTCX.js";import{M as ut}from"./message-square-DoOdNzx2.js";import{T as mt}from"./trash-2-C3BtOLsp.js";import{S as pt}from"./sparkles-QMiFSVZb.js";/**
...
`}),t}}function ks(s){const e=/```integration-action\s*\n([\s\S]*?)```/,t=s.match(e);if(!t)return null;try{return JSON.parse(t[1].trim())}catch{return null}}function Ps(s){return s.replace(/```integration-action\s*\n[\s\S]*?```/g,"").trim()}function Ms(s,e){const t=s.toLowerCase(),r=e.toLowerCase(),o=(t.includes("auditor")||t.includes("verificar")||t.includes("status")||t.includes("listar"))&&(t.includes("integra")||t.includes("conex")||t.includes("plataforma")),i=r.includes("vou")&&(r.includes("auditor")||r.includes("verificar"));if(!o||!i)return null;const l={facebook:"META_ADS",instagram:"META_ADS",meta:"META_ADS",google:"GOOGLE_ADS",linkedin:"LINKEDIN_ADS",tiktok:"TIKTOK_ADS",twitter:"TWITTER_ADS"};for(const[v,u]of Object.entries(l))if(t.includes(v))return{action:"audit",platform:u};return{action:"audit_all"}}const _s=["Criar campanha de Facebook Ads","Analisar performance da última semana","Sugerir otimizações"],me=500,ar=()=>{const[s,e]=d.useState(""),[t,r]=d.useState(!0),[o,i]=d.useState(null),[l,v]=d.useState(!1),[u,b]=d.useState([]),[g,h]=d.useState(""),[m,f]=d.useState([]),[S,I]=d.useState([]),[j,L]=d.useState([]),[w,C]=d.useState(null),[q,P]=d.useState(""),[G,Y]=d.useState([]),[xe,Le]=d.useState(!1),[Ls,fa]=d.useState(null),ve=d.useRef(null),be=d.useRef([]),x=Be(n=>n.user),qe=Be(n=>n.isAuthenticated),$e=Ja(),Se=U(n=>n.conversations),R=U(n=>n.activeConversationId),Ae=U(n=>n.setActiveConversationId),ie=U(n=>n.isAssistantTyping),ze=U(n=>n.setAssistantTyping),X=U(n=>n.addMessage);U(n=>n.deleteConversation),U(n=>n.createNewConversation);const xa=et(n=>n.addCampaign),va=Ze(n=>n.aiSystemPrompt),ba=Ze(n=>n.aiInitialGreetings),Ge=d.useRef(null),Fe=d.useRef(null),Sa=n=>{if(!n||typeof n!="string")return n;let c=n;return c.split(`
`).slice(0,3).join(`
...
`),c=c.trim(),!c||c.length<10||/^[\s\{\}\[\]\,\:\"]+$/.test(c)?(console.warn("⚠️ cleanTechnicalLogs removeu todo conteúdo, retornando original"),n):c},Aa=async(n,c,p)=>{const A=`msg-${Date.now()+1}`;let E="";X(n,c,{id:A,role:"assistant",content:""});const M=p.split(" "),$=2;for(let T=0;T<M.length;T+=$){const F=M.slice(T,T+$).join(" ");E+=(T>0?" ":"")+F,X(n,c,{id:A,role:"assistant",content:E}),await new Promise(Z=>setTimeout(Z,20))}X(n,c,{id:A,role:"assistant",content:p})},{toast:y}=at(),B=Se.find(n=>n.id===R);d.useEffect(()=>{(!qe||!x)&&(console.warn("⚠️ Usuário não autenticado, redirecionando para login..."),$e("/login",{replace:!0}))},[qe,x,$e]);const Ea=()=>{var n;(n=Ge.current)==null||n.scrollIntoView({behavior:"smooth"})};d.useEffect(Ea,[B==null?void 0:B.messages,ie]),d.useEffect(()=>{(async()=>{if(x)try{const{data:c,error:p}=await D.from("GlobalAiConnection").select("id, systemPrompt, initialGreetings").eq("isActive",!0).limit(1).single();if(p){console.error("Erro ao buscar IA:",p);return}const A=c==null?void 0:c.id;if(A){const{data:E,error:M}=await D.from("GlobalAiConnection").select("systemPrompt, initialGreetings").eq("id",A).single();if(M){console.error("Erro ao buscar config da IA:",M);return}E&&i({systemPrompt:E.systemPrompt||va,initialGreetings:E.initialGreetings||ba})}}catch(c){console.error("Erro ao carregar IA Global:",c)}})()},[x==null?void 0:x.id]),d.useEffect(()=>{if(B&&B.messages.length===0){const n=ss();setTimeout(()=>{x&&X(x.id,R,{id:`greeting-${Date.now()}`,role:"assistant",content:n})},500)}},[R,B==null?void 0:B.messages.length]);const wa=n=>{const c=/ZIP_DOWNLOAD:\s*({[^}]+})/g,p=n.match(c);return p&&p.forEach(A=>{try{const E=A.replace("ZIP_DOWNLOAD:","").trim(),M=JSON.parse(E);I($=>[...$,M])}catch(E){console.error("Erro ao processar download ZIP:",E)}}),n.replace(c,"").trim()},ya=n=>{const c=/SUPER_AI_EXECUTION:\s*({[^}]+})/g,p=n.match(c);return p&&p.forEach(A=>{try{const E=A.replace("SUPER_AI_EXECUTION:","").trim(),M=JSON.parse(E);L($=>[...$,M])}catch(E){console.error("Erro ao processar execução Super AI:",E)}}),n.replace(c,"").trim()},Ee=async()=>{if(s.trim()===""||!R||s.length>me)return;const n=s,c=n.toLowerCase();if(c.includes("pesquis")||c.includes("busca")||c.includes("google")||c.includes("internet")){C("web_search");let p=n;if(c.includes("pesquis")){const A=n.match(/pesquis[ae]\s+(.+)/i);p=A?A[1]:n}P(`Pesquisando na web sobre: "${p}"`),Y(["Google Search","Exa AI","Tavily"])}else if(c.includes("baix")||c.includes("rasp")||c.includes("scrape")){C("web_scraping");const p=n.match(/https?:\/\/[^\s]+/i);P(p?`Raspando dados de: ${p[0]}`:"Raspando dados...")}else c.includes("python")||c.includes("calcule")||c.includes("execute código")?(C("python_exec"),P("Executando código Python para processar dados...")):(C(null),P("Processando sua solicitação..."),Y([]));x&&X(x.id,R,{id:`msg-${Date.now()}`,role:"user",content:n}),e(""),r(!1),ze(!0);try{const p=Se.find(N=>N.id===R),A=(o==null?void 0:o.systemPrompt)||ts,E=ms+`
...
`+A,M=((p==null?void 0:p.messages)||[]).slice(-20).map(N=>({role:N.role,content:N.content})),T=(await st(n,R,M,E)).response,F=es(T);if(F)try{x&&(await xa(x.id,{name:F.data.name,platform:F.data.platform,status:"Pausada",budgetTotal:F.data.budgetTotal,budgetSpent:0,impressions:0,clicks:0,conversions:0,startDate:F.data.startDate,endDate:F.data.endDate||"",ctr:0,cpc:0}),y({title:"🎉 Campanha Criada!",description:`A campanha "${F.data.name}" foi criada com sucesso.`}))}catch(N){console.error("Error creating campaign from AI:",N),y({title:"Erro ao criar campanha",description:"Não foi possível criar a campanha automaticamente.",variant:"destructive"})}let Z="";if(x){const N=new ps(x.id),H=gs(T);if(H){const O=await N.executeSQL(H);y({title:O.success?"✅ SQL Executado":"❌ Erro SQL",description:O.message,variant:O.success?"default":"destructive"})}const te=hs(T);if(te){const O=await N.analyzeSystem(te.type,te.period);y({title:O.success?"📊 Análise Concluída":"❌ Erro",description:O.message,variant:O.success?"default":"destructive"})}const le=fs(T);if(le){const O=await N.manageIntegration(le.action,le.platform,le.credentials);y({title:O.success?"🔗 Integração Atualizada":"❌ Erro",description:O.message,variant:O.success?"default":"destructive"})}const de=xs(T);if(de){const O=await N.getMetrics(de.metric,de.aggregation,de.groupBy);y({title:O.success?"📈 Métricas Obtidas":"❌ Erro",description:O.message,variant:O.success?"default":"destructive"})}let J=ks(T);if(J||(J=Ms(n,T)),J){const O=new Os(x.id);let W;switch(J.action){case"audit":J.platform&&(W=await O.auditIntegration(J.platform));break;case"audit_all":W=await O.auditAll();break;case"list_status":W=await O.listStatus();break;case"test":case"capabilities":case"diagnose":W={success:!0,message:`Ação "${J.action}" detectada. Implementação em andamento.`};break}W&&(Z=`

`+W.message,y({title:W.success?"✅ Ação Executada":"❌ Erro",description:W.success?"Auditoria concluída com sucesso":W.error||"Erro ao executar ação",variant:W.success?"default":"destructive"}))}}const z=Ts(T);if(z&&x)try{if(z.action==="connect"){const{authUrl:N}=await ue.generateOAuthUrl(z.slug,x.id),H=Ne[z.slug];x&&X(x.id,R,{id:`msg-${Date.now()+1}`,role:"assistant",content:`Para conectar ${H.name}, clique no link abaixo:

🔗 [Autorizar ${H.name}](${N})

O link abrirá em uma nova aba para você autorizar o acesso.`}),window.open(N,"_blank"),y({title:"🔗 Link de Autorização",description:`Clique no link para conectar ${H.name}`});return}else if(z.action==="disconnect"){await ue.disconnect(x.id,z.slug);const N=Ne[z.slug];y({title:"✅ Desconectado",description:`${N.name} foi desconectado com sucesso.`})}else if(z.action==="status")if(z.slug){const N=await ue.getIntegrationStatus(x.id,z.slug),H=Ne[z.slug];y({title:`${H.name}`,description:N!=null&&N.isConnected?"✅ Conectado":"❌ Não conectado"})}else{const N=await ue.listIntegrations(x.id),H=N.filter(te=>te.isConnected).length;y({title:"📊 Status das Integrações",description:`${H} de ${N.length} integrações conectadas`})}}catch(N){console.error("Erro ao processar integração:",N),x&&X(x.id,R,{id:`msg-${Date.now()+2}`,role:"assistant",content:`❌ **Erro ao conectar integração**

${N.message||"Erro ao processar comando de integração"}`}),y({title:"❌ Erro na Integração",description:"Verifique as instruções no chat",variant:"destructive"})}console.log("📝 Resposta original da IA:",T.substring(0,200));let ce=wa(T);ce=ya(ce),console.log("📦 Após processar ZIP:",ce.substring(0,200));let V=as(ce);V=vs(V),V=Is(V),V=Ps(V),console.log("🧹 Após limpar blocos:",V.substring(0,200)),V=Sa(V),console.log("🔧 Após cleanTechnicalLogs:",V.substring(0,200));const Ve=Ss(V+Z,{addEmojis:!0,improveMarkdown:!0,removeTechnicalLogs:!0,addSectionDividers:!1});console.log("✨ Resposta final formatada:",Ve.substring(0,200)),x&&await Aa(x.id,R,Ve)}catch(p){console.error("Erro ao chamar IA:",p),y({title:"Erro ao gerar resposta",description:p.message||"Não foi possível obter resposta da IA. Verifique sua chave de API.",variant:"destructive"}),x&&X(x.id,R,{id:`msg-${Date.now()+1}`,role:"assistant",content:"❌ Desculpe, ocorreu um erro ao processar sua mensagem. Por favor, verifique se sua chave de API está configurada corretamente nas configurações."})}finally{ze(!1)}},Na=n=>{e(n)},Ca=()=>{var n;(n=Fe.current)==null||n.click()},ja=async()=>{try{const n=await navigator.mediaDevices.getUserMedia({audio:!0}),c=new MediaRecorder(n);ve.current=c,be.current=[],c.ondataavailable=p=>{p.data.size>0&&be.current.push(p.data)},c.onstop=async()=>{const p=new Blob(be.current,{type:"audio/webm"});fa(p),await Ia(p),n.getTracks().forEach(A=>A.stop())},c.start(),Le(!0),y({title:"🎤 Gravando...",description:"Clique novamente para parar e enviar."})}catch(n){console.error("Erro ao iniciar gravação:",n),y({title:"❌ Erro",description:"Não foi possível acessar o microfone.",variant:"destructive"})}},Ta=()=>{ve.current&&xe&&(ve.current.stop(),Le(!1))},Ia=async n=>{if(!(!x||!R))try{y({title:"📤 Enviando áudio...",description:"Aguarde..."});const c=new File([n],`audio-${Date.now()}.webm`,{type:"audio/webm"}),p=`${x.id}/audio-${Date.now()}-${Math.random().toString(36).substring(7)}.webm`,{data:A,error:E}=await D.storage.from("chat-attachments").upload(p,c,{cacheControl:"3600",upsert:!1});if(E)throw E;const{data:{publicUrl:M}}=D.storage.from("chat-attachments").getPublicUrl(p),$=`[🎤 Mensagem de áudio](${M})`;e(T=>T?`${T}

${$}`:$),y({title:"✅ Áudio enviado!",description:"O áudio foi adicionado à mensagem."})}catch(c){console.error("Erro ao enviar áudio:",c),y({title:"❌ Erro",description:"Não foi possível enviar o áudio.",variant:"destructive"})}},Ra=async n=>{var p;const c=(p=n.target.files)==null?void 0:p[0];if(!(!c||!x||!R))try{y({title:"📤 Upload iniciado",description:`Enviando "${c.name}"...`});const A=c.name.split(".").pop(),E=`${x.id}/${Date.now()}-${Math.random().toString(36).substring(7)}.${A}`,{data:M,error:$}=await D.storage.from("chat-attachments").upload(E,c,{cacheControl:"3600",upsert:!1});if($)throw $;const{data:{publicUrl:T}}=D.storage.from("chat-attachments").getPublicUrl(E),{error:F}=await D.from("ChatAttachment").insert({messageId:"",fileName:c.name,fileType:c.type,fileUrl:T,fileSize:c.size});F&&console.error("Erro ao salvar anexo:",F);const Z=c.type.startsWith("image/")?`![${c.name}](${T})`:`[${c.name}](${T})`,z=s?`${s}

${Z}`:Z;e(""),z.trim()&&R&&Ee(),y({title:"✅ Arquivo enviado!",description:`${c.name} foi enviado com sucesso.`})}catch(A){console.error("Erro ao fazer upload:",A),y({title:"❌ Erro ao enviar arquivo",description:A.message||"Não foi possível enviar o arquivo.",variant:"destructive"})}finally{n.target&&(n.target.value="")}},Da=async n=>{try{const{data:c,error:p}=await D.from("ChatMessage").select("id, role, content, createdAt").eq("conversationId",n).order("createdAt",{ascending:!0});if(p)throw p;const A=(c||[]).map(E=>({id:E.id,role:E.role,content:E.content,timestamp:new Date(E.createdAt)}));U.getState().setConversationMessages(n,A),Ae(n),console.log(`✅ ${A.length} mensagens carregadas da conversa ${n}`)}catch(c){console.error("Erro ao carregar mensagens:",c),y({title:"Erro",description:"Não foi possível carregar mensagens.",variant:"destructive"})}};d.useEffect(()=>{(async()=>{if(!x)return;await U.getState().loadConversations(x.id);const{data:c}=await D.from("ChatConversation").select("id").eq("userId",x.id).limit(1);(!c||c.length===0)&&await Ue()})()},[x]);const Ue=async()=>{try{if(!x)return;const n=crypto.randomUUID(),c=new Date().toISOString(),{error:p}=await D.from("ChatConversation").insert({id:n,userId:x.id,title:"🆕 Nova Conversa",createdAt:c,updatedAt:c});if(p)throw p;Ae(n),await U.getState().loadConversations(x.id),y({title:"✅ Nova conversa criada!",description:"Comece um novo chat do zero."})}catch(n){console.error("Erro ao criar nova conversa:",n),y({title:"Erro",description:n.message||"Não foi possível criar nova conversa.",variant:"destructive"})}},Oa=async n=>{try{await D.from("ChatMessage").delete().eq("conversationId",n);const{error:c}=await D.from("ChatConversation").delete().eq("id",n);if(c)throw c;R===n&&Ae(null),await U.getState().loadConversations(x.id),y({title:"🗑️ Conversa deletada",description:"A conversa foi removida com sucesso."})}catch(c){console.error("Erro ao deletar conversa:",c),y({title:"Erro",description:"Não foi possível deletar a conversa.",variant:"destructive"})}};return a.jsxs("div",{className:"h-[calc(100vh-80px)] flex flex-col md:flex-row",children:[a.jsxs("div",{className:`${t?"fixed md:relative inset-0 md:inset-auto w-full md:w-72 z-50 md:z-auto":"hidden md:w-0"} transition-all duration-300 bg-gray-50 md:border-r border-gray-200 flex flex-col overflow-hidden`,children:[a.jsxs("div",{className:"p-4 border-b border-gray-200",children:[a.jsxs("div",{className:"flex items-center justify-between mb-3",children:[a.jsx("h2",{className:"text-sm font-semibold text-gray-700",children:"Conversas"}),a.jsx(k,{onClick:()=>r(!1),variant:"ghost",size:"sm",className:"h-7 w-7 p-0",children:a.jsx(He,{className:"h-4 w-4"})})]}),a.jsxs(k,{onClick:Ue,className:"w-full gap-2",size:"sm",children:[a.jsx(dt,{className:"h-4 w-4"}),"Nova Conversa"]})]}),a.jsx("div",{className:"flex-1 overflow-y-auto p-2 space-y-1",children:Se.map(n=>a.jsxs("div",{className:`group relative flex items-center gap-2 p-3 rounded-lg hover:bg-gray-100 cursor-pointer transition-colors ${R===n.id?"bg-blue-50 border border-blue-200":"bg-white"}`,onClick:()=>{R!==n.id&&Da(n.id)},children:[a.jsx(ut,{className:"h-4 w-4 text-gray-500 flex-shrink-0"}),a.jsxs("div",{className:"flex-1 min-w-0",children:[a.jsx("p",{className:"text-sm font-medium text-gray-900 truncate",children:n.title}),a.jsx("p",{className:"text-xs text-gray-500 truncate",children:n.messages&&n.messages.length>0?n.messages[n.messages.length-1].content.substring(0,50)+"...":"Sem mensagens ainda"})]}),a.jsx(k,{onClick:c=>{c.stopPropagation(),Oa(n.id)},variant:"ghost",size:"sm",className:"h-7 w-7 p-0 opacity-0 group-hover:opacity-100 transition-opacity",children:a.jsx(mt,{className:"h-3.5 w-3.5 text-red-500"})})]},n.id))})]}),a.jsxs("div",{className:"flex-1 flex flex-col min-w-0 w-full",children:[a.jsx("div",{className:"border-b border-gray-200 bg-white/80 backdrop-blur-xl p-3 md:p-4",children:a.jsxs("div",{className:"flex items-center justify-between gap-2",children:[a.jsxs("div",{className:"flex items-center gap-3",children:[a.jsx(k,{onClick:()=>r(!t),variant:"ghost",size:"sm",className:"h-8 w-8 md:h-9 md:w-9 p-0 flex-shrink-0",children:t?a.jsx(He,{className:"h-4 w-4 md:h-5 md:w-5"}):a.jsx(tt,{className:"h-4 w-4 md:h-5 md:w-5"})}),a.jsx("div",{className:"p-1.5 sm:p-2 rounded-lg bg-gradient-to-r from-blue-500 to-purple-500",children:a.jsx(ye,{className:"h-4 w-4 sm:h-6 sm:w-6 text-white"})}),a.jsxs("div",{children:[a.jsx("h1",{className:"text-base sm:text-xl font-bold text-gray-900",children:"Chat com IA"}),a.jsx("p",{className:"text-xs sm:text-sm text-gray-500",children:"Assistente inteligente"})]})]}),a.jsxs(se,{className:"bg-gradient-to-r from-green-500 to-emerald-500 text-xs sm:text-sm",children:[a.jsx(pt,{className:"h-3 w-3 mr-1"}),"Online"]})]})}),a.jsx("div",{className:"flex-1 overflow-y-auto p-3 md:p-4 space-y-3 md:space-y-4 overscroll-contain",children:B?a.jsxs(a.Fragment,{children:[(l||u.length>0)&&a.jsx(rs,{isSearching:l,searchResults:u,searchQuery:g}),m.length>0&&a.jsx(os,{sources:m,isSearching:l}),S.length>0&&a.jsx("div",{className:"mb-4",children:a.jsx(cs,{downloads:S})}),j.length>0&&a.jsx("div",{className:"mb-4 space-y-4",children:j.map((n,c)=>a.jsx(us,{toolName:n.toolName,parameters:n.parameters,userId:(x==null?void 0:x.id)||"",conversationId:R||"",onComplete:p=>{console.log("Execução Super AI concluída:",p)}},c))}),B.messages.map(n=>{var p;const c=(p=n.content)==null?void 0:p.match(/INTEGRATION_CONNECT:(\w+):([^🔗]+)/);if(c&&n.role==="assistant"){const[,A,E]=c,M=n.content.replace(/🔗 \*\*INTEGRATION_CONNECT:[^🔗]+🔗\*\* 🔗\n\n/,"");return a.jsx("div",{className:"flex justify-start",children:a.jsxs("div",{className:"max-w-[80%]",children:[a.jsx(Q,{className:"bg-white mb-2",children:a.jsx(K,{className:"p-4",children:a.jsxs("div",{className:"flex items-start gap-2",children:[a.jsx(ye,{className:"h-5 w-5 text-blue-600 mt-1 flex-shrink-0"}),a.jsx("div",{className:"flex-1 whitespace-pre-wrap break-words text-sm text-gray-900",children:M})]})})}),a.jsx(ns,{platform:A,platformName:E.trim(),onSkip:()=>{console.log("Conexão pulada:",A)},onSuccess:()=>{console.log("Conectado com sucesso:",A)}})]})},n.id)}return a.jsx("div",{className:`flex ${n.role==="user"?"justify-end":"justify-start"} mb-3`,children:a.jsx(Q,{className:`w-full max-w-[90%] sm:max-w-[85%] md:max-w-[75%] lg:max-w-[70%] ${n.role==="user"?"bg-gradient-to-r from-blue-500 to-purple-500 text-white":"bg-white"}`,children:a.jsxs(K,{className:"p-3 md:p-4",children:[a.jsxs("div",{className:"flex items-start gap-2",children:[n.role==="assistant"&&a.jsx(ye,{className:"h-4 w-4 md:h-5 md:w-5 text-blue-600 mt-0.5 flex-shrink-0"}),a.jsx("div",{className:`flex-1 whitespace-pre-wrap break-words text-sm md:text-base ${n.role==="user"?"text-white":"text-gray-900"}`,style:{wordBreak:"break-word",overflowWrap:"anywhere",maxWidth:"100%"},children:n.content})]}),a.jsx("div",{className:`text-xs mt-2 ${n.role==="user"?"text-white/70":"text-gray-500"}`,children:n.timestamp?new Date(n.timestamp).toLocaleTimeString("pt-BR"):""})]})})},n.id)}),ie&&a.jsx(ct,{isThinking:ie,currentTool:w,reasoning:q,sources:G,status:"thinking"}),ie&&a.jsx("div",{className:"flex justify-start",children:a.jsx(Q,{className:"bg-white",children:a.jsx(K,{className:"p-4",children:a.jsxs("div",{className:"flex items-center gap-2",children:[a.jsx(re,{className:"h-4 w-4 animate-spin text-blue-600"}),a.jsx("span",{className:"text-sm text-gray-500",children:"Processando..."})]})})})}),a.jsx("div",{ref:Ge})]}):a.jsx("div",{className:"flex items-center justify-center h-full text-muted-foreground",children:a.jsx("p",{children:"Selecione ou crie uma conversa para começar."})})}),a.jsxs("div",{className:"border-t border-gray-200 p-3 md:p-4 bg-white/80 backdrop-blur-xl flex-shrink-0",children:[a.jsx("div",{className:"hidden sm:flex gap-2 mb-2",children:_s.map(n=>a.jsx(k,{variant:"outline",size:"sm",onClick:()=>Na(n),className:"text-xs",children:n},n))}),a.jsxs("div",{className:"relative",children:[a.jsx(Ot,{value:s,onChange:n=>e(n.target.value),onKeyDown:n=>{n.key==="Enter"&&!n.shiftKey&&window.innerWidth>768&&(n.preventDefault(),Ee())},placeholder:"Digite sua mensagem...",className:"w-full resize-none rounded-lg border bg-background p-3 pr-20 md:pr-24 min-h-[44px] md:min-h-[48px] text-sm md:text-base",minRows:1,maxRows:window.innerWidth<768?3:5,maxLength:me}),a.jsxs("div",{className:"absolute right-2 top-1/2 -translate-y-1/2 flex gap-1",children:[a.jsx("input",{type:"file",ref:Fe,onChange:Ra,className:"hidden"}),a.jsx(Je,{children:a.jsxs(ea,{children:[a.jsx(aa,{asChild:!0,children:a.jsx(k,{type:"button",size:"icon",variant:"ghost",onClick:Ca,className:"h-8 w-8 md:h-9 md:w-9 flex-shrink-0 touch-manipulation",children:a.jsx(ht,{className:"h-4 w-4"})})}),a.jsx(Oe,{children:"Anexar arquivo"})]})}),a.jsx(Je,{children:a.jsxs(ea,{children:[a.jsx(aa,{asChild:!0,children:a.jsx(k,{type:"button",size:"icon",variant:"ghost",onClick:xe?Ta:ja,disabled:!R,className:`h-8 w-8 md:h-9 md:w-9 flex-shrink-0 touch-manipulation ${xe?"text-red-500 animate-pulse":""}`,children:a.jsx(gt,{className:"h-4 w-4"})})}),a.jsx(Oe,{children:"Gravar áudio"})]})}),a.jsx(k,{type:"submit",size:"icon",onClick:Ee,disabled:s.trim()===""||!R,className:"h-8 w-8 md:h-9 md:w-9 flex-shrink-0 touch-manipulation",children:a.jsx(rt,{className:"h-4 w-4"})})]})]}),a.jsxs("p",{className:pe("text-xs text-right mt-1",s.length>me?"text-destructive":"text-muted-foreground"),children:[s.length," / ",me]})]})]})]})};export{ar as default};
