...
class OmnibrainService {
  ...
  async execute(input: OmnibrainTaskInput): Promise<OmnibrainResponse> {
    ...
    try {
      ...
      const payload = {
        ...
        files: input.files || [],
        options: {
          max_retries: input.options?.max_retries || DEFAULT_MAX_RETRIES,
          timeout: input.options?.timeout || this.defaultTimeout,
          enable_hybrid: input.options?.enable_hybrid ?? true,
   ...
   */
  async executeSimple(command: string, context?: Record<string, any>): Promise<any> {
 ...
 */
export function formatOmnibrainResult(result: OmnibrainExecutionResult): string {
...
export function detectTaskType(command: string): TaskType {
  ...
  const taskKeywords: Record<TaskType, string[]> = {
    [TaskType.IMAGE_PROCESSING]: ['imagem', 'image', 'foto', 'picture', 'resize', 'crop'],
    [TaskType.VIDEO_PROCESSING]: ['vídeo', 'video', 'filme', 'movie'],
    [TaskType.AUDIO_PROCESSING]: ['áudio', 'audio', 'som', 'sound', 'música'],
    [TaskType.WEB_SCRAPING]: ['scraping', 'scrape', 'extrair', 'extract', 'crawl'],
    [TaskType.DATA_ANALYSIS]: ['analise', 'analyze', 'análise', 'dados', 'data'],
