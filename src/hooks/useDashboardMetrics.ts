  ...
  useEffect(() => {
    ...
    const fetchMetrics = async () => {
      try {
        ...
        const totalClicks = currentCampaigns?.reduce((sum, c) => sum + (c.clicks || 0), 0) || 0;
        const totalConversions = currentCampaigns?.reduce((sum, c) => sum + (c.conversions || 0), 0) || 0;
        const totalSpent = currentCampaigns?.reduce((sum, c) => sum + (c.budgetSpent || 0), 0) || 0;
        const totalRevenue = currentCampaigns?.reduce((sum, c) => sum + (c.revenue || 0), 0) || 0;

        // Calcular métricas anteriores
        const prevTotalCampaigns = previousCampaigns?.length || 0;
        const prevTotalClicks = previousCampaigns?.reduce((sum, c) => sum + (c.clicks || 0), 0) || 0;
        const prevTotalConversions = previousCampaigns?.reduce((sum, c) => sum + (c.conversions || 0), 0) || 0;
        const prevTotalRevenue = previousCampaigns?.reduce((sum, c) => sum + (c.revenue || 0), 0) || 0;
