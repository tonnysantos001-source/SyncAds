...
import { shopifyCollectionsApi } from "@/lib/api/shopifyCollections";
...
const CollectionsPage = () => {
  ...
  const handleSyncShopify = async () => {
    ...
    try {
      ...
      toast({
        title: "Sincronizando...",
        description: "Buscando coleções da Shopify",
      });

      const result = await shopifySyncApi.syncProducts(user.id);

      if (result.success) {
        toast({
          title: "Sincronização concluída!",
          description: "Produtos e coleções sincronizados com sucesso",
        });
    ...
    try {
      ...
      if (editingCollection) {
        ...
        toast({
          ...
          description: "A coleção está pronta para receber produtos.",
        });
      }
    ...
    <div className="space-y-6">
      ...
      >
        <Card className="border-0 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl shadow-lg">
          <CardHeader>
            <CardTitle className="text-xl font-bold bg-gradient-to-r from-indigo-600 to-fuchsia-600 bg-clip-text text-transparent">
              Lista de Coleções
            </CardTitle>
          </CardHeader>
          <CardContent>
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  Comece criando a primeira coleção
                </p>
