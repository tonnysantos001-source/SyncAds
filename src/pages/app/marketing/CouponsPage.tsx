...
import { Plus, Tag, Search, Edit, Trash2, Percent, DollarSign, Ticket } from 'lucide-react';
...
const CouponsPage = () => {
  ...
  const loadCoupons = async () => {
    try {
      ...
      const data = await marketingApi.coupons.getAll(user.organizationId);
      setCoupons(data);
    ...
    <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input type="number" step="0.01" value={formData.value}
                    onChange={(e) => setFormData({ ...formData, value: parseFloat(e.target.value) })} required />
                </div>
                <div>
                  <Label>Compra Mínima</Label>
                  <Input type="number" step="0.01" value={formData.minPurchaseAmount}
                ...
                <div>
                  ...
                  <Input type="datetime-local" value={formData.startsAt}
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent><div className="text-2xl font-bold">{activeCoupons}</div></CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
      ...
      <div className="relative">
        ...
        <Input placeholder="Buscar cupons..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9" />
      </div>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-sm text-muted-foreground mb-4">{searchTerm ? 'Tente ajustar sua busca' : 'Comece criando seu primeiro cupom'}</p>
              {!searchTerm && <Button onClick={resetForm}><Plus className="mr-2 h-4 w-4" />Criar Primeiro Cupom</Button>}
            </div>
          ) : (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {filteredCoupons.map((coupon) => (
                  <TableRow key={coupon.id}>
                    <TableCell><code className="font-mono font-bold">{coupon.code}</code></TableCell>
                    <TableCell>{coupon.name}</TableCell>
                    <TableCell>
                      <Badge variant="outline">
                        {coupon.type === 'PERCENTAGE' && <Percent className="h-3 w-3 mr-1" />}
                        {coupon.type === 'FIXED_AMOUNT' && <DollarSign className="h-3 w-3 mr-1" />}
                        {coupon.type === 'PERCENTAGE' ? 'Porcentagem' : coupon.type === 'FIXED_AMOUNT' ? 'Valor Fixo' : 'Frete Grátis'}
                      </Badge>
                    </TableCell>
                    <TableCell>
                      {coupon.type === 'PERCENTAGE' ? `${coupon.value}%` : coupon.type === 'FIXED_AMOUNT' ? `R$ ${coupon.value.toFixed(2)}` : '-'}
                    </TableCell>
                    <TableCell>{coupon.usageCount}{coupon.usageLimit && ` / ${coupon.usageLimit}`}</TableCell>
                    <TableCell><Badge variant={coupon.isActive ? 'default' : 'secondary'}>{coupon.isActive ? 'Ativo' : 'Inativo'}</Badge></TableCell>
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button variant="ghost" size="icon" onClick={() => handleEdit(coupon)}><Edit className="h-4 w-4" /></Button>
                        <Button variant="ghost" size="icon" onClick={() => handleDelete(coupon.id)}><Trash2 className="h-4 w-4" /></Button>
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
