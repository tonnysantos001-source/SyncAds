...
import { Plus, Search, Edit, Trash2, Users, MessageSquare, TrendingUp } from 'lucide-react';
...
const SocialProofPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingProof) {
        ...
        if (error) throw error;
        toast({ title: 'Prova social criada!', description: 'A notificação está pronta para exibição.' });
      }
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case 'RECENT_PURCHASE': return <TrendingUp className="h-4 w-4" />;
      case 'VISITOR_COUNT': return <Users className="h-4 w-4" />;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Notificações em tempo real para aumentar confiança</p>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-lg">
            <DialogHeader>
              <DialogTitle>{editingProof ? 'Editar Prova Social' : 'Nova Prova Social'}</DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {formData.type === 'RECENT_PURCHASE' && 'Ex: "João acabou de comprar há 5 minutos"'}
                  {formData.type === 'VISITOR_COUNT' && 'Ex: "23 pessoas estão visualizando este produto"'}
                  {formData.type === 'REVIEW' && 'Ex: "★★★★★ Produto excelente! - Maria S."'}
                </p>
              </div>
              <div>
              ...
              <DialogFooter>
                ...
                <Button type="submit">{editingProof ? 'Atualizar' : 'Criar'}</Button>
              </DialogFooter>
            </form>
          </DialogContent>
        </Dialog>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent><div className="text-2xl font-bold">{socialProofs.length}</div></CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent><div className="text-2xl font-bold">{activeProofs}</div></CardContent>
        </Card>
      </div>

      <div className="relative">
        <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
        <Input placeholder="Buscar provas sociais..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9" />
      </div>

      <Card>
        <CardHeader><CardTitle>Lista de Provas Sociais</CardTitle></CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <h3 className="text-lg font-semibold">Nenhuma prova social encontrada</h3>
              <p className="text-sm text-muted-foreground mb-4">
                {searchTerm ? 'Tente ajustar sua busca' : 'Comece criando a primeira prova social'}
              </p>
              {!searchTerm && <Button onClick={resetForm}><Plus className="mr-2 h-4 w-4" />Criar Primeira Prova Social</Button>}
            </div>
          ) : (
            <Table>
              <TableHeader>
                  ...
                  <TableRow key={proof.id}>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm">{getTypeLabel(proof.type)}</span>
                      </div>
                    </TableCell>
                    <TableCell className="max-w-md truncate">{proof.message}</TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button variant="ghost" size="icon" onClick={() => handleEdit(proof)}><Edit className="h-4 w-4" /></Button>
                        <Button variant="ghost" size="icon" onClick={() => handleDelete(proof.id)}><Trash2 className="h-4 w-4" /></Button>
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
