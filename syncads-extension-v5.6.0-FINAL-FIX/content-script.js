// ============================================
// SYNCADS EXTENSION - CONTENT SCRIPT v5.0
// Robust Token Detection & Background Communication
// ============================================

// ✅ GUARD: Prevenir execução múltipla (IDEMPOTÊNCIA)
(function () {
  'use strict';

  if (window.__SYNCADS_CONTENT_SCRIPT_LOADED__) {
    console.log('⚠️ [SyncAds] Content script already loaded, skipping reinjection...');
    return;
  }

  window.__SYNCADS_CONTENT_SCRIPT_LOADED__ = true;
  ...
  console.log("🚀 SyncAds Extension v5.0 - Content Script Active", {
    domain: currentDomain,
    isSyncAdsSite: isSyncAdsSite,
    url: window.location.href,
  });

  console.log("🚀 SyncAds Content Script v5.0 - Initializing on:", currentDomain);
  ...
  // Gerar Correlation ID único por sessão do content script
  const CORRELATION_ID = `cs_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`;
  ...
  const Logger = {
    _log: (level, message, data = {}) => {
      ...
      console[method](`${emoji} [${CORRELATION_ID}] ${message}`, logEntry);
    },
    ...
    error: (msg, error, data) => Logger._log('error', msg, {
      error: error?.message,
      stack: error?.stack,
      ...data
    }),
  ...
  /**
   * WRAPPER SEGURO: Garante resposta única em handlers async
   */
  function createAsyncMessageHandler(handler) {
    return (message, sender, sendResponse) => {
      ...
      Promise.resolve(handler(message, sender, safeRespond))
        .catch(error => {
      ...
      switch (action) {
        ...
        case "WAIT":
          if (params.selector) {
            ...
            try {
              await waitForElement(params.selector, params.timeout || 20000);
        ...
        case "CHECK_DOC_STATUS":
          return await detectGoogleDocsCreated(params.timeout || 20000);
  ...
  /**
   * ========================================
   * DETECTOR CANÔNICO: DOCUMENT_READY
   * ========================================
   * 
   * Sinal único e determinístico para Google Docs.
   * 
   * CRITÉRIOS:
   * 1. URL: /document/d/{docId}/edit
   * 2. Título ≠ "Google Docs"
   * 3. Editor: .kix-canvas-tile-content OU [contenteditable]
   * 4. DOM estável por ≥ 2000ms
   * 
   * SINAIS EMITIDOS:
   * - DOCUMENT_READY (sucesso)
   * - DOCUMENT_READY_TIMEOUT (falha)
   */
  async function detectDocumentReady(timeout = 20000) {
    // ✅ GUARD: Prevenir múltiplas execuções
    if (window.__SYNCADS_DOC_READY_SENT__) {
      Logger.info('⚠️ DOCUMENT_READY already sent, skipping...');
      return { success: true, already_sent: true };
    }

    return new Promise((resolve) => {
      const STABILITY_THRESHOLD = 2000; // 2 segundos de estabilidade
      let lastMutation = Date.now();
      let stabilityTimer;

      // Função de verificação de condições
      const check = () => {
        const url = location.href;
        return {
          url,
          title: document.title,
          urlOk: /\/document\/d\/.+\/edit/.test(url),
          titleOk: document.title !== 'Google Docs' && document.title.trim().length > 0,
          editorOk: !!(
      ...
      const confirm = (state) => {
        window.__SYNCADS_DOC_READY_SENT__ = true;

        const docId = state.url.split('/d/')[1]?.split('/')[0] || 'unknown';
        ...
        const signal = {
          ...
          payload: {
            ...
            title: state.title,
            docId: docId,
            stability_duration: STABILITY_THRESHOLD
          }
        };

        Logger.success('✅ [CANONICAL] DOCUMENT_READY signal sent', signal.payload);
      ...
      // MutationObserver para detectar estabilidade
      const observer = new MutationObserver(() => {
        ...
        if (state.urlOk && state.titleOk && state.editorOk) {
      ...
      // Check inicial (pode já estar pronto)
      const initial = check();
      if (initial.urlOk && initial.titleOk && initial.editorOk) {
        stabilityTimer = setTimeout(() => {
          observer.disconnect();
          confirm(initial);
        }, STABILITY_THRESHOLD);
      ...
      // Timeout geral: enviar sinal de falha
      setTimeout(() => {
        observer.disconnect();
        if (stabilityTimer) clearTimeout(stabilityTimer);

        // Se ainda não enviou o sinal de sucesso, enviar timeout
        if (!window.__SYNCADS_DOC_READY_SENT__) {
          window.__SYNCADS_DOC_READY_SENT__ = true;

          const timeoutSignal = {
            type: 'DOCUMENT_READY_TIMEOUT',
            timestamp: Date.now(),
            payload: {
              url: location.href,
              title: document.title,
          ...
          Logger.error('❌ [CANONICAL] DOCUMENT_READY_TIMEOUT', timeoutSignal.payload);
  ...
  const detectGoogleDocsCreated = detectDocumentReady;

  /**
   * ========================================
   * BUSCA RESILIENTE: MERCADO LIVRE
   * ========================================
   * 
   * Estratégia independente de idioma/região.
   * 
   * ESTRATÉGIAS (em ordem):
   * 1. Preencher input + form.submit()
   * 2. Preencher input + Enter key
   * 3. Verificar resultados por mudança de URL ou presença de elementos
   * 
   * NÃO depende de aria-label ou texto do botão.
   */
  async function executeMercadoLivreSearch(query) {
    Logger.info(`🔍 [ML] Executando busca: ${query}`);

    try {
      // Estratégia 1: Encontrar input de busca (semântico)
      const input =
        document.querySelector('input[name="as_word"]') ||
        document.querySelector('header input[type="text"]') ||
        document.querySelector('input[aria-label*="uscar"]') || // Fallback: busca parcial
      ...
      Logger.info(`📝 [ML] Input encontrado: ${input.name || input.placeholder}`);
      ...
      // Estratégia 2: Submeter formulário (preferencial)
      const form = input.closest('form');
      if (form) {
        Logger.info('📋 [ML] Submetendo formulário');
        form.submit();
      } else {
        // Estratégia 3: Pressionar Enter
        Logger.info('⌨️ [ML] Pressionando Enter');
      ...
      // Verificação de sucesso: presença de resultados
      const results =
        document.querySelector('.ui-search-results') ||
        document.querySelector('[data-testid="results"]') ||
        document.querySelector('.results-list') ||
        document.querySelector('section ol'); // Fallback genérico

      const urlChanged = window.location.href.includes('search') ||
        window.location.href.includes('buscador') ||
        window.location.href.includes(encodeURIComponent(query));

      if (results || urlChanged) {
        Logger.success(`✅ [ML] Busca executada com sucesso`);
        return { success: true, results_found: !!results, url_changed: urlChanged };
      }

      throw new Error('Resultados de busca não detectados');

    } catch (error) {
      Logger.error('[ML] Falha na busca', error);
      throw error;
    }
  }
