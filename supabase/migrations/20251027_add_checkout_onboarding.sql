...
-- Data: 27/10/2025
-- Descrição: Campos para billing, domain e shipping
-- =====================================================
...
CREATE TABLE IF NOT EXISTS "ShippingMethod" (
  ...
  "appliesTo" TEXT[] DEFAULT ARRAY['ALL'], -- 'ALL', 'SPECIFIC_PRODUCTS', 'SPECIFIC_COLLECTIONS'
  "appliesToIds" UUID[],
...
-- 8. COMENTÁRIOS
COMMENT ON COLUMN "Organization"."stripeCustomerId" IS 'ID do cliente no Stripe';
COMMENT ON COLUMN "Organization"."subscriptionId" IS 'ID da subscription no Stripe';
COMMENT ON COLUMN "Organization"."subscriptionStatus" IS 'Status da subscription';
COMMENT ON COLUMN "Organization"."domain" IS 'Domínio configurado pelo cliente';
COMMENT ON COLUMN "Organization"."domainVerified" IS 'Se o domínio foi verificado';
COMMENT ON COLUMN "Organization"."domainVerificationToken" IS 'Token para verificação do domínio';
