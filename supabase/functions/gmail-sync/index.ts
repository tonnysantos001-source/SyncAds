...
serve(async (req) => {
  ...
  try {
    ...
    const { data: integration } = await supabase.from("GmailIntegration").select("*").eq("id", integrationId).single();
    ...
    const items: any[] = [];
    await supabase.from("GmailIntegration").update({ lastSyncAt: new Date().toISOString(), lastSyncStatus: "success" }).eq("id", integration.id);
