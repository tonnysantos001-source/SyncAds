...
serve(async (req) => {
  ...
  try {
    ...
    const { data: integration } = await supabase.from("TiktokadsIntegration").select("*").eq("id", integrationId).single();
    ...
    const items: any[] = [];
    await supabase.from("TiktokadsIntegration").update({ lastSyncAt: new Date().toISOString(), lastSyncStatus: "success" }).eq("id", integration.id);
