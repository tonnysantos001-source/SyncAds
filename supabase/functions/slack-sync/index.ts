...
serve(async (req) => {
  ...
  try {
    ...
    const { data: integration } = await supabase.from("SlackIntegration").select("*").eq("id", integrationId).single();
    ...
    const items: any[] = [];
    await supabase.from("SlackIntegration").update({ lastSyncAt: new Date().toISOString(), lastSyncStatus: "success" }).eq("id", integration.id);
