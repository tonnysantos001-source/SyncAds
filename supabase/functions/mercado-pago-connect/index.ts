...
serve(async (req) => {
  ...
  try {
    ...
    const testUrl = `https://api.mercadopago.com/test`;
    ...
    const integrationId = `${user.id}-${accessToken}`;
    ...
    if (existingIntegration) {
      ...
      return new Response(
        JSON.stringify({
          success: true,
          message: "Mercado Pago integration updated successfully",
      ...
      return new Response(
        JSON.stringify({
          success: true,
          message: "Mercado Pago integration connected successfully",
          integrationId,
        }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
