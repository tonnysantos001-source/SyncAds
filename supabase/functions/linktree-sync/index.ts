...
serve(async (req) => {
  ...
  try {
    ...
    const { data: integration } = await supabase.from("LinktreeIntegration").select("*").eq("id", integrationId).single();
    ...
    const items: any[] = [];
    await supabase.from("LinktreeIntegration").update({ lastSyncAt: new Date().toISOString(), lastSyncStatus: "success" }).eq("id", integration.id);
