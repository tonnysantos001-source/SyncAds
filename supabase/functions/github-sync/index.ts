...
serve(async (req) => {
  ...
  try {
    ...
    const { data: integration } = await supabase.from("GithubIntegration").select("*").eq("id", integrationId).single();
    ...
    const items: any[] = [];
    await supabase.from("GithubIntegration").update({ lastSyncAt: new Date().toISOString(), lastSyncStatus: "success" }).eq("id", integration.id);
