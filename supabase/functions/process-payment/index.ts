...
async function processAsaasPayment(
  ...
  try {
    ...
    const charge = {
      ...
      billingType: request.paymentMethod === 'pix' ? 'PIX' : request.paymentMethod === 'boleto' ? 'BOLETO' : 'CREDIT_CARD',
...
serve(async (req) => {
  ...
  try {
    ...
    switch (gateway.type) {
      case 'stripe':
        paymentResponse = await processStripePayment(paymentRequest, gatewayConfig)
        break
      case 'mercadopago':
        paymentResponse = await processMercadoPagoPayment(paymentRequest, gatewayConfig)
        break
      case 'pagseguro':
        paymentResponse = await processPagSeguroPayment(paymentRequest, gatewayConfig)
        break
      case 'paypal':
        paymentResponse = await processPayPalPayment(paymentRequest, gatewayConfig)
        break
      case 'asaas':
        paymentResponse = await processAsaasPayment(paymentRequest, gatewayConfig)
        break
      default:
