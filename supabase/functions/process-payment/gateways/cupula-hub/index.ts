...
export class CpulaHubGateway extends BaseGateway {
  name = "Cúpula Hub";
  slug = "cupula-hub";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.apiKey) {
        return {
          isValid: false,
          message: "apiKey is required for Cúpula Hub",
        };
      }
      if (!credentials.hubId) {
        return {
          isValid: false,
          message: "hubId is required for Cúpula Hub",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Cúpula Hub payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Cúpula Hub webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
