...
export class BrazaPayGateway extends BaseGateway {
  name = "Braza Pay";
  slug = "braza-pay";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.apiKey) {
        return {
          isValid: false,
          message: "apiKey is required for Braza Pay",
        };
      }
      if (!credentials.accountId) {
        return {
          isValid: false,
          message: "accountId is required for Braza Pay",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Braza Pay payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Braza Pay webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
