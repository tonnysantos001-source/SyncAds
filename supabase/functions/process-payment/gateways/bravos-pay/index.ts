...
export class BravosPayGateway extends BaseGateway {
  name = "Bravos Pay";
  slug = "bravos-pay";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.merchantId) {
        return {
          isValid: false,
          message: "merchantId is required for Bravos Pay",
        };
      }
      if (!credentials.apiToken) {
        return {
          isValid: false,
          message: "apiToken is required for Bravos Pay",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Bravos Pay payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Bravos Pay webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
