...
export class FusionPayGateway extends BaseGateway {
  name = "FusionPay";
  slug = "fusionpay";
  supportedMethods = [
  ...
  endpoints: GatewayEndpoints = {
    production: "https://api.fusionpay.com/v1",
    sandbox: "https://sandbox.fusionpay.com/v1",
  };
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.apiKey) {
        return {
          isValid: false,
          message: "apiKey is required for FusionPay",
        };
      }
      if (!credentials.secretKey) {
        return {
          isValid: false,
          message: "secretKey is required for FusionPay",
        };
      }
      if (!credentials.environment) {
        return {
          isValid: false,
          message: "environment is required for FusionPay",
        };
      }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "FusionPay webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
