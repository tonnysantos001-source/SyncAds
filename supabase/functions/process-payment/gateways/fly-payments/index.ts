...
export class FlyPaymentsGateway extends BaseGateway {
  name = "Fly Payments";
  slug = "fly-payments";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.apiToken) {
        return {
          isValid: false,
          message: "apiToken is required for Fly Payments",
        };
      }
      if (!credentials.sellerId) {
        return {
          isValid: false,
          message: "sellerId is required for Fly Payments",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Fly Payments payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Fly Payments webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
