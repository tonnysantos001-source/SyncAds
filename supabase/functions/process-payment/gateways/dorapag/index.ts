...
export class DorapagGateway extends BaseGateway {
  name = "Dorapag";
  slug = "dorapag";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.apiKey) {
        return {
          isValid: false,
          message: "apiKey is required for Dorapag",
        };
      }
      if (!credentials.secretKey) {
        return {
          isValid: false,
          message: "secretKey is required for Dorapag",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Dorapag payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Dorapag webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
