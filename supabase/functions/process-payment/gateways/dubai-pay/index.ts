...
export class DubaiPayGateway extends BaseGateway {
  name = "Dubai Pay";
  slug = "dubai-pay";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.merchantId) {
        return {
          isValid: false,
          message: "merchantId is required for Dubai Pay",
        };
      }
      if (!credentials.apiToken) {
        return {
          isValid: false,
          message: "apiToken is required for Dubai Pay",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Dubai Pay payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Dubai Pay webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
