...
export class EverPayGateway extends BaseGateway {
  name = "Ever Pay";
  slug = "ever-pay";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.apiKey) {
        return {
          isValid: false,
          message: "apiKey is required for Ever Pay",
        };
      }
      if (!credentials.accountId) {
        return {
          isValid: false,
          message: "accountId is required for Ever Pay",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Ever Pay payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Ever Pay webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
