...
export class CenturionPayGateway extends BaseGateway {
  name = "Centurion Pay";
  slug = "centurion-pay";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.merchantId) {
        return {
          isValid: false,
          message: "merchantId is required for Centurion Pay",
        };
      }
      if (!credentials.apiToken) {
        return {
          isValid: false,
          message: "apiToken is required for Centurion Pay",
        };
      }
      ...
      const response = await fetch(`${endpoint}/health`, {
        method: "GET",
        headers: {
          "Authorization": `Bearer ${config.credentials.merchantId}`,
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Centurion Pay payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Centurion Pay webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
