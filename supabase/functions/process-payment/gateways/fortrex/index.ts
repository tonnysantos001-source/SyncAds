...
export class FortrexGateway extends BaseGateway {
  name = "Fortrex";
  slug = "fortrex";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.apiKey) {
        return {
          isValid: false,
          message: "apiKey is required for Fortrex",
        };
      }
      if (!credentials.merchantCode) {
        return {
          isValid: false,
          message: "merchantCode is required for Fortrex",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Fortrex payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Fortrex webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
