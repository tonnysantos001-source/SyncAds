...
export class BlackcatGateway extends BaseGateway {
  name = "Blackcat";
  slug = "blackcat";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.apiKey) {
        return {
          isValid: false,
          message: "apiKey is required for Blackcat",
        };
      }
      if (!credentials.secretKey) {
        return {
          isValid: false,
          message: "secretKey is required for Blackcat",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Blackcat payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Blackcat webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
