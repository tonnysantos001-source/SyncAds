...
export class FlowspayGateway extends BaseGateway {
  name = "Flowspay";
  slug = "flowspay";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.clientId) {
        return {
          isValid: false,
          message: "clientId is required for Flowspay",
        };
      }
      if (!credentials.clientSecret) {
        return {
          isValid: false,
          message: "clientSecret is required for Flowspay",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Flowspay payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Flowspay webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
