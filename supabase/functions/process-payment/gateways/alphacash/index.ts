...
export class AlphacashGateway extends BaseGateway {
  name = "Alphacash";
  slug = "alphacash";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.apiKey) {
        return {
          isValid: false,
          message: "apiKey is required for Alphacash",
        };
      }
      if (!credentials.secretKey) {
        return {
          isValid: false,
          message: "secretKey is required for Alphacash",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Alphacash payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Alphacash webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
