...
export class AxelpayGateway extends BaseGateway {
  name = "Axelpay";
  slug = "axelpay";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.token) {
        return {
          isValid: false,
          message: "token is required for Axelpay",
        };
      }
      if (!credentials.storeId) {
        return {
          isValid: false,
          message: "storeId is required for Axelpay",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Axelpay payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Axelpay webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
