...
export class SafetyPayGateway extends BaseGateway {
  name = "SafetyPay";
  slug = "safetypay";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.apiKey) {
        return {
          isValid: false,
          message: "apiKey is required for SafetyPay",
        };
      }
      if (!credentials.signatureKey) {
        return {
          isValid: false,
          message: "signatureKey is required for SafetyPay",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process SafetyPay payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "SafetyPay webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
