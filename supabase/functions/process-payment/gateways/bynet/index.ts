...
export class BynetGateway extends BaseGateway {
  name = "Bynet";
  slug = "bynet";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.clientId) {
        return {
          isValid: false,
          message: "clientId is required for Bynet",
        };
      }
      if (!credentials.clientSecret) {
        return {
          isValid: false,
          message: "clientSecret is required for Bynet",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Bynet payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Bynet webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
