...
export class AtlasPayGateway extends BaseGateway {
  name = "Atlas Pay";
  slug = "atlas-pay";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.apiId) {
        return {
          isValid: false,
          message: "apiId is required for Atlas Pay",
        };
      }
      if (!credentials.apiSecret) {
        return {
          isValid: false,
          message: "apiSecret is required for Atlas Pay",
        };
      }
      ...
      const response = await fetch(`${endpoint}/health`, {
        method: "GET",
        headers: {
          "Authorization": `Basic ${btoa(`${config.credentials.apiId}:${config.credentials.apiSecret}`)}`,
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Atlas Pay payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Atlas Pay webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
