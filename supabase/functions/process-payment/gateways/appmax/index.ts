...
export class AppmaxGateway extends BaseGateway {
  name = "Appmax";
  slug = "appmax";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.apiKey) {
        return {
          isValid: false,
          message: "apiKey is required for Appmax",
        };
      }
      if (!credentials.accountId) {
        return {
          isValid: false,
          message: "accountId is required for Appmax",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Appmax payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Appmax webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
