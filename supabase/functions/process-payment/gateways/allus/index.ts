...
export class AllusGateway extends BaseGateway {
  name = "Allus";
  slug = "allus";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.apiKey) {
        return {
          isValid: false,
          message: "apiKey is required for Allus",
        };
      }
      if (!credentials.merchantId) {
        return {
          isValid: false,
          message: "merchantId is required for Allus",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Allus payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Allus webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
