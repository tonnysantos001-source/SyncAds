...
export class DiasmarketplaceGateway extends BaseGateway {
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.apiKey) {
        return {
          isValid: false,
          message: "apiKey is required for Diasmarketplace",
        };
      }
      if (!credentials.storeId) {
        return {
          isValid: false,
          message: "storeId is required for Diasmarketplace",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Diasmarketplace payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Diasmarketplace webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
