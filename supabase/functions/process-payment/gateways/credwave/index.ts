...
export class CredwaveGateway extends BaseGateway {
  name = "Credwave";
  slug = "credwave";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.token) {
        return {
          isValid: false,
          message: "token is required for Credwave",
        };
      }
      if (!credentials.sellerId) {
        return {
          isValid: false,
          message: "sellerId is required for Credwave",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Credwave payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Credwave webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
