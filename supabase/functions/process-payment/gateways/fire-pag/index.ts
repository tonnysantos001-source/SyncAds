...
export class FirePagGateway extends BaseGateway {
  name = "Fire Pag";
  slug = "fire-pag";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.token) {
        return {
          isValid: false,
          message: "token is required for Fire Pag",
        };
      }
      if (!credentials.storeId) {
        return {
          isValid: false,
          message: "storeId is required for Fire Pag",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Fire Pag payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Fire Pag webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
