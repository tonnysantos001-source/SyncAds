...
export class AxionPayGateway extends BaseGateway {
  name = "Axion Pay";
  slug = "axion-pay";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.clientId) {
        return {
          isValid: false,
          message: "clientId is required for Axion Pay",
        };
      }
      if (!credentials.clientSecret) {
        return {
          isValid: false,
          message: "clientSecret is required for Axion Pay",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Axion Pay payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Axion Pay webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
