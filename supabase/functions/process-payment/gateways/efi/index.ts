...
export class EfGateway extends BaseGateway {
  name = "Efí";
  slug = "efi";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.clientId) {
        return {
          isValid: false,
          message: "clientId is required for Efí",
        };
      }
      if (!credentials.clientSecret) {
        return {
          isValid: false,
          message: "clientSecret is required for Efí",
        };
      }
      if (!credentials.environment) {
        return {
          isValid: false,
          message: "environment is required for Efí",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Efí payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Efí webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
