...
export class FastPayGateway extends BaseGateway {
  name = "Fast Pay";
  slug = "fast-pay";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.apiKey) {
        return {
          isValid: false,
          message: "apiKey is required for Fast Pay",
        };
      }
      if (!credentials.merchantId) {
        return {
          isValid: false,
          message: "merchantId is required for Fast Pay",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Fast Pay payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Fast Pay webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
