...
export class FivepayGateway extends BaseGateway {
  name = "Fivepay";
  slug = "fivepay";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.apiKey) {
        return {
          isValid: false,
          message: "apiKey is required for Fivepay",
        };
      }
      if (!credentials.partnerId) {
        return {
          isValid: false,
          message: "partnerId is required for Fivepay",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Fivepay payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Fivepay webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
