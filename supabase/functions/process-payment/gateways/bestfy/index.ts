...
export class BestfyGateway extends BaseGateway {
  name = "Bestfy";
  slug = "bestfy";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.token) {
        return {
          isValid: false,
          message: "token is required for Bestfy",
        };
      }
      if (!credentials.environment) {
        return {
          isValid: false,
          message: "environment is required for Bestfy",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Bestfy payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Bestfy webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
