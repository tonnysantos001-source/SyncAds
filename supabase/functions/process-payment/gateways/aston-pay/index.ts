...
export class AstonPayGateway extends BaseGateway {
  name = "Aston Pay";
  slug = "aston-pay";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.merchantCode) {
        return {
          isValid: false,
          message: "merchantCode is required for Aston Pay",
        };
      }
      if (!credentials.apiKey) {
        return {
          isValid: false,
          message: "apiKey is required for Aston Pay",
        };
      }
      ...
      const response = await fetch(`${endpoint}/health`, {
        method: "GET",
        headers: {
          "X-API-Key": config.credentials.merchantCode,
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Aston Pay payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Aston Pay webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
