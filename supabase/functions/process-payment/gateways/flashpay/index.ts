...
export class FlashPayGateway extends BaseGateway {
  name = "FlashPay";
  slug = "flashpay";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.apiKey) {
        return {
          isValid: false,
          message: "apiKey is required for FlashPay",
        };
      }
      if (!credentials.merchantId) {
        return {
          isValid: false,
          message: "merchantId is required for FlashPay",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process FlashPay payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "FlashPay webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
