...
export class AzcendGateway extends BaseGateway {
  name = "Azcend";
  slug = "azcend";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.apiKey) {
        return {
          isValid: false,
          message: "apiKey is required for Azcend",
        };
      }
      if (!credentials.merchantId) {
        return {
          isValid: false,
          message: "merchantId is required for Azcend",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Azcend payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Azcend webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
