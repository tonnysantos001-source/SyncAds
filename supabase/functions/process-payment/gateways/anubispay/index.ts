...
export class AnubisPayGateway extends BaseGateway {
  name = "AnubisPay";
  slug = "anubispay";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.merchantId) {
        return {
          isValid: false,
          message: "merchantId is required for AnubisPay",
        };
      }
      if (!credentials.apiToken) {
        return {
          isValid: false,
          message: "apiToken is required for AnubisPay",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process AnubisPay payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "AnubisPay webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
