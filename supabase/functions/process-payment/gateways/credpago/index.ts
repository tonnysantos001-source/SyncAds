...
export class CredpagoGateway extends BaseGateway {
  name = "Credpago";
  slug = "credpago";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.apiKey) {
        return {
          isValid: false,
          message: "apiKey is required for Credpago",
        };
      }
      if (!credentials.partnerId) {
        return {
          isValid: false,
          message: "partnerId is required for Credpago",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Credpago payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Credpago webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
