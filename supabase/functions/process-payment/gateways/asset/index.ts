...
export class AssetGateway extends BaseGateway {
  name = "Asset";
  slug = "asset";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.publicKey) {
        return {
          isValid: false,
          message: "publicKey is required for Asset",
        };
      }
      if (!credentials.privateKey) {
        return {
          isValid: false,
          message: "privateKey is required for Asset",
        };
      }
      ...
      const response = await fetch(`${endpoint}/health`, {
        method: "GET",
        headers: {
          "X-API-Key": config.credentials.publicKey,
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Asset payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Asset webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
