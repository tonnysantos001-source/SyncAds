...
export class CartheroGateway extends BaseGateway {
  name = "Carthero";
  slug = "carthero";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.apiKey) {
        return {
          isValid: false,
          message: "apiKey is required for Carthero",
        };
      }
      if (!credentials.storeId) {
        return {
          isValid: false,
          message: "storeId is required for Carthero",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Carthero payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Carthero webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
