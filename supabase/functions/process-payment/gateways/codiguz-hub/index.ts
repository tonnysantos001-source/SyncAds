...
export class CodiguzHubGateway extends BaseGateway {
  name = "Codiguz Hub";
  slug = "codiguz-hub";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.apiToken) {
        return {
          isValid: false,
          message: "apiToken is required for Codiguz Hub",
        };
      }
      if (!credentials.merchantCode) {
        return {
          isValid: false,
          message: "merchantCode is required for Codiguz Hub",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Codiguz Hub payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Codiguz Hub webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
