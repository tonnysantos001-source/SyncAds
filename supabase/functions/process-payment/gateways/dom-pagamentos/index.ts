...
export class DomPagamentosGateway extends BaseGateway {
  name = "Dom Pagamentos";
  slug = "dom-pagamentos";
  supportedMethods = [
  ...
  async validateCredentials(
    ...
    try {
      if (!credentials.token) {
        return {
          isValid: false,
          message: "token is required for Dom Pagamentos",
        };
      }
      if (!credentials.merchantId) {
        return {
          isValid: false,
          message: "merchantId is required for Dom Pagamentos",
        };
      }
  ...
  async processPayment(
    ...
    try {
      ...
      return this.createErrorResponse(error, "Failed to process Dom Pagamentos payment");
    }
  }
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.log("error", "Dom Pagamentos webhook error", { error: error.message });
      return {
        success: false,
        processed: false,
        message: error.message,
