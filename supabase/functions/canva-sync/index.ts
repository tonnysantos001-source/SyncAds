...
serve(async (req) => {
  ...
  try {
    ...
    const { data: integration } = await supabase.from("CanvaIntegration").select("*").eq("id", integrationId).single();
    ...
    const items: any[] = [];
    await supabase.from("CanvaIntegration").update({ lastSyncAt: new Date().toISOString(), lastSyncStatus: "success" }).eq("id", integration.id);
