...
serve(async (req) => {
  ...
  try {
    ...
    const { data: integration } = await supabase.from("PostgresqlIntegration").select("*").eq("id", integrationId).single();
    ...
    const items: any[] = [];
    await supabase.from("PostgresqlIntegration").update({ lastSyncAt: new Date().toISOString(), lastSyncStatus: "success" }).eq("id", integration.id);
