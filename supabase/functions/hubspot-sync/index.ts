...
serve(async (req) => {
  ...
  try {
    ...
    const { data: integration } = await supabase.from("HubspotIntegration").select("*").eq("id", integrationId).single();
    ...
    const items: any[] = [];
    await supabase.from("HubspotIntegration").update({ lastSyncAt: new Date().toISOString(), lastSyncStatus: "success" }).eq("id", integration.id);
