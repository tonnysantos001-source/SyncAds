...
export interface DomCommand {
  type: 'NAVIGATE' | 'CLICK' | 'FILL_FORM' | 'SCREENSHOT' | 'EXECUTE_JS' | 'READ_TEXT' | 'SCROLL_TO' | 'WAIT';
...
const NAVIGATION_PATTERNS = [
  // Abrir sites
  { regex: /(?:abra?|abrir|acessar|vai para|vá para|ir para|navegue? para|visite?|entre em)\s+(?:o\s+)?(?:site\s+)?(?:do\s+)?(?:da\s+)?(.+?)(?:\s+(?:por favor|pfv|pf))?$/i, confidence: 0.95 },
  { regex: /(?:quero|gostaria de|pode|poderia)\s+(?:abrir|acessar|ir para|entrar)\s+(?:o\s+)?(?:site\s+)?(?:do\s+)?(?:da\s+)?(.+?)$/i, confidence: 0.90 },
  { regex: /(?:me leve|leve-me|me leva)\s+(?:para|ao|à)\s+(?:o\s+)?(?:site\s+)?(?:do\s+)?(?:da\s+)?(.+?)$/i, confidence: 0.90 },
  ...
  // Sites conhecidos
  { regex: /(?:abra?|acessar?|ir para)\s+(?:o\s+)?(facebook|fb|instagram|insta|ig|youtube|yt|twitter|x|linkedin|google|gmail|whatsapp|tiktok|amazon|mercado\s*livre|olx|shopee|aliexpress)/i, confidence: 0.98 },
];
...
const CLICK_PATTERNS = [
  { regex: /(?:clique?|clicar|aperte?|apertar|pressione?|pressionar)\s+(?:no|na|em|sobre)\s+(.+?)$/i, confidence: 0.90 },
  { regex: /(?:pode|poderia)\s+(?:clicar|apertar|pressionar)\s+(?:no|na|em)\s+(.+?)$/i, confidence: 0.85 },
];

const FILL_PATTERNS = [
  { regex: /(?:preencha?|preencher|preenchimento|digite?|digitar|escreva?|escrever)\s+(.+?)\s+(?:no|na|em)\s+(.+?)$/i, confidence: 0.85 },
  { regex: /(?:insira?|inserir|coloque?|colocar)\s+(.+?)\s+(?:no|na|em)\s+(.+?)$/i, confidence: 0.85 },
];

const SCREENSHOT_PATTERNS = [
  { regex: /(?:tire?|tirar|capture?|capturar|fazer|faz|faça)\s+(?:uma?\s+)?(?:foto|print|screenshot|captura|imagem)\s*(?:da\s+tela)?/i, confidence: 0.95 },
  { regex: /(?:pode|poderia)\s+(?:tirar|capturar)\s+(?:uma?\s+)?(?:screenshot|print|foto)/i, confidence: 0.90 },
];

const READ_PATTERNS = [
  { regex: /(?:leia?|ler|pegue?|pegar|extraia?|extrair|busque?|buscar)\s+(?:o\s+)?(?:texto|conteúdo|informação|dados?)\s+(?:de|do|da)\s+(.+?)$/i, confidence: 0.85 },
  { regex: /(?:qual|quais|me diga?|me diz|mostre?|mostrar)\s+(?:o|a|os|as)\s+(.+?)$/i, confidence: 0.70 },
];

const SCROLL_PATTERNS = [
  { regex: /(?:role?|rolar|desça?|descer|suba?|subir)\s+(?:a\s+página|página|para)\s+(.+?)$/i, confidence: 0.85 },
  { regex: /(?:vá|ir)\s+(?:para|ao|até)\s+(?:o\s+)?(?:topo|fim|final|início|começo)/i, confidence: 0.90 },
];
...
function detectNavigation(message: string): DomCommand | null {
  for (const pattern of NAVIGATION_PATTERNS) {
    ...
    if (match) {
      ...
      // Se é uma URL direta
      if (captured && (captured.startsWith('http') || captured.startsWith('www'))) {
