...
serve(async (req) => {
  ...
  try {
    ...
    const integrationId = `${user.id}-Gmail`;
    await supabase.from("GmailIntegration").upsert({ id: integrationId, userId: user.id, apiKey, credentials, isActive: true, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }, { onConflict: "userId" });
