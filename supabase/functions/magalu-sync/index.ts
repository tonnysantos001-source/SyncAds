import { serve } from "https://deno.land/std@0.168.0/http/server.ts";
import { createClient } from "https://esm.sh/@supabase/supabase-js@2";

const corsHeaders = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Headers":
    "authorization, x-client-info, apikey, content-type",
};

serve(async (req) => {
  if (req.method === "OPTIONS") {
    return new Response("ok", { headers: corsHeaders });
  }

  try {
    const supabaseUrl = Deno.env.get("SUPABASE_URL")!;
    const supabaseKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
    const supabase = createClient(supabaseUrl, supabaseKey);

    const {
      integrationId,
      action = "sync-all",
      limit = 100,
    } = await req.json();

    if (!integrationId) {
      return new Response(
        JSON.stringify({ success: false, error: "integrationId is required" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
      );
    }

    const { data: integration, error: integrationError } = await supabase
      .from("MagaluIntegration")
      .select("*")
      .eq("id", integrationId)
      .eq("isActive", true)
      .single();

    if (integrationError || !integration) {
      return new Response(
        JSON.stringify({
          success: false,
          error: "Integration not found or inactive",
        }),
        {
          status: 404,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
      );
    }

    const { data: syncLog } = await supabase
      .from("MagaluSyncLog")
      .insert({
        id: crypto.randomUUID(),
        userId: integration.userId,
        integrationId: integration.id,
        syncType: action,
        status: "started",
        startedAt: new Date().toISOString(),
      })
      .select()
      .single();

    const results: any = {};
    let hasError = false;
    let errorMessage = "";

    try {
      // Sync products
      if (action === "sync-all" || action === "sync-products") {
        const productsResult = await syncProducts(supabase, integration, limit);
        results.products = productsResult;
        if (!productsResult.success) {
          hasError = true;
          errorMessage = productsResult.error || "Failed to sync products";
        }
      }

      // Sync orders
      if (action === "sync-all" || action === "sync-orders") {
        const ordersResult = await syncOrders(supabase, integration, limit);
        results.orders = ordersResult;
        if (!ordersResult.success) {
          hasError = true;
          errorMessage = ordersResult.error || "Failed to sync orders";
        }
      }

      await supabase
        .from("MagaluSyncLog")
        .update({
          status: hasError ? "completed_with_errors" : "completed",
          completedAt: new Date().toISOString(),
          errorMessage: hasError ? errorMessage : null,
          details: results,
        })
        .eq("id", syncLog.id);

      await supabase
        .from("MagaluIntegration")
        .update({
          lastSyncAt: new Date().toISOString(),
          lastSyncStatus: hasError ? "error" : "success",
        })
        .eq("id", integration.id);

      return new Response(
        JSON.stringify({ success: !hasError, results, syncLogId: syncLog.id }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    } catch (error: any) {
      await supabase
        .from("MagaluSyncLog")
        .update({
          status: "failed",
          completedAt: new Date().toISOString(),
          errorMessage: error.message,
          details: results,
        })
        .eq("id", syncLog.id);

      throw error;
    }
  } catch (error: any) {
    console.error("Magazine Luiza sync error:", error);
    return new Response(
      JSON.stringify({ success: false, error: error.message }),
      {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      },
    );
  }
});

async function syncProducts(supabase: any, integration: any, limit: number) {
  try {
    const products: any[] = [];
    let offset = 0;
    let hasMore = true;

    while (hasMore && products.length < limit) {
      const url = `https://api.magazineluiza.com.br/seller/v1/products?offset=${offset}&limit=50`;

      const response = await fetch(url, {
        headers: {
          "Content-Type": "application/json",
          "x-api-key": integration.apiKey,
          "x-seller-id": integration.sellerId,
        },
      });

      if (!response.ok) {
        return {
          success: false,
          error: `API error: ${response.status}`,
          count: 0,
        };
      }

      const data = await response.json();
      const pageProducts = data.products || [];

      if (pageProducts.length === 0) {
        hasMore = false;
      } else {
        products.push(...pageProducts);
        offset += 50;

        if (pageProducts.length < 50) {
          hasMore = false;
        }
      }

      // Rate limiting - Magalu permite 5 requisições por segundo
      await new Promise((resolve) => setTimeout(resolve, 250));
    }

    if (products.length > 0) {
      const productsToInsert = products.map((p: any) => ({
        id: `magalu-${integration.id}-${p.id}`,
        userId: integration.userId,
        integrationId: integration.id,
        productId: String(p.id),
        sku: p.sku || "",
        name: p.title || "",
        description: p.description || "",
        price: parseFloat(p.price || 0),
        salePrice: p.sale_price ? parseFloat(p.sale_price) : null,
        stock: parseInt(p.stock || 0),
        status: p.status || "active",
        category: p.category || "",
        brand: p.brand || "",
        gtin: p.gtin || "",
        weight: p.weight ? parseFloat(p.weight) : null,
        height: p.height ? parseFloat(p.height) : null,
        width: p.width ? parseFloat(p.width) : null,
        length: p.length ? parseFloat(p.length) : null,
        images: p.images || [],
        attributes: p.attributes || {},
        magaluData: p,
        lastSyncAt: new Date().toISOString(),
      }));

      const { error } = await supabase
        .from("MagaluProduct")
        .upsert(productsToInsert, { onConflict: "id" });

      if (error) {
        return { success: false, error: error.message, count: 0 };
      }
    }

    return { success: true, count: products.length, synced: products.length };
  } catch (error: any) {
    return { success: false, error: error.message, count: 0 };
  }
}

async function syncOrders(supabase: any, integration: any, limit: number) {
  try {
    const orders: any[] = [];
    let offset = 0;
    let hasMore = true;

    // Buscar pedidos dos últimos 30 dias
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - 30);
    const startDateStr = startDate.toISOString().split("T")[0];

    while (hasMore && orders.length < limit) {
      const url = `https://api.magazineluiza.com.br/seller/v1/orders?offset=${offset}&limit=50&start_date=${startDateStr}`;

      const response = await fetch(url, {
        headers: {
          "Content-Type": "application/json",
          "x-api-key": integration.apiKey,
          "x-seller-id": integration.sellerId,
        },
      });

      if (!response.ok) {
        return {
          success: false,
          error: `API error: ${response.status}`,
          count: 0,
        };
      }

      const data = await response.json();
      const pageOrders = data.orders || [];

      if (pageOrders.length === 0) {
        hasMore = false;
      } else {
        orders.push(...pageOrders);
        offset += 50;

        if (pageOrders.length < 50) {
          hasMore = false;
        }
      }

      // Rate limiting - Magalu permite 5 requisições por segundo
      await new Promise((resolve) => setTimeout(resolve, 250));
    }

    if (orders.length > 0) {
      const ordersToInsert = orders.map((o: any) => ({
        id: `magalu-${integration.id}-${o.id}`,
        userId: integration.userId,
        integrationId: integration.id,
        orderId: String(o.id),
        orderNumber: String(o.order_number),
        status: o.status || "",
        statusName: o.status_name || "",
        total: parseFloat(o.total_amount || 0),
        subtotal: parseFloat(o.subtotal_amount || 0),
        discount: parseFloat(o.discount_amount || 0),
        shipping: parseFloat(o.freight_amount || 0),
        currency: "BRL",
        customerName: o.customer?.name || "",
        customerEmail: o.customer?.email || "",
        customerPhone: o.customer?.phone || "",
        customerDocument: o.customer?.document || "",
        shippingAddress: o.shipping_address || {},
        billingAddress: o.billing_address || {},
        items: o.items || [],
        paymentMethod: o.payment?.method || "",
        paymentStatus: o.payment?.status || "",
        invoiceKey: o.invoice?.key || "",
        trackingCode: o.tracking?.code || "",
        trackingUrl: o.tracking?.url || "",
        magaluData: o,
        orderDate: o.created_at || new Date().toISOString(),
        lastSyncAt: new Date().toISOString(),
      }));

      const { error } = await supabase
        .from("MagaluOrder")
        .upsert(ordersToInsert, { onConflict: "id" });

      if (error) {
        return { success: false, error: error.message, count: 0 };
      }
    }

    return { success: true, count: orders.length, synced: orders.length };
  } catch (error: any) {
    return { success: false, error: error.message, count: 0 };
  }
}
