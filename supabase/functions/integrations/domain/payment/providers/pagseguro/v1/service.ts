...
export class Service extends BaseGateway {
  ...
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      // PagSeguro retorna 200 OK ou 401 Unauthorized se as chaves forem inválidas.
      if (res.ok || res.status === 200 || res.status === 201) {
        return { isValid: true, message: "Conexão estabelecida com PagSeguro com sucesso." };
      } else {
        const body = await res.json().catch(() => ({}));
        return { 
          isValid: false, 
          message: `Conexão rejeitada pelo PagSeguro. HTTP status ${res.status}: ${body.message || "E-mail ou Token inválidos"}` 
        };
      }
    } catch (err: any) {
      return { isValid: false, message: `Erro de rede ao conectar com PagSeguro: ${err.message}` };
    }
  }
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `PagSeguro rejeitou o PIX: ${body.error_messages?.[0]?.description || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `PagSeguro rejeitou o cartão: ${body.error_messages?.[0]?.description || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `PagSeguro rejeitou o boleto: ${body.error_messages?.[0]?.description || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
  ...
  async consultPayment(gatewayTransactionId: string, config: IntegrationConfig): Promise<PaymentStatusResponse> {
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(`Erro ao consultar pagamento no PagSeguro (${res.status}): ${body.message || "Erro desconhecido"}`);
      }
    } catch (err: any) {
