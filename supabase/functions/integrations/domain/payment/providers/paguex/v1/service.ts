...
export class Service extends BaseGateway {
  readonly name = "Pague-X";
  readonly slug = "paguex";
  ...
  /**
   * Valida as credenciais da Pague-X.
   */
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          isValid: false,
          message: "Credenciais Pague-X inválidas. Verifique a publicKey e secretKey.",
        };
      }

      return {
        isValid: true,
        message: "Credenciais Pague-X validadas com sucesso.",
      };
    } catch (err: any) {
      return {
        isValid: true,
  ...
  /**
   * Processa pagamentos via Pague-X.
   */
    ...
    try {
      ...
      if (!res.ok) {
        return {
          success: false,
          status: "failed",
          message: `Pague-X rejeitou a transação (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
      ...
      return {
        success: false,
        status: "failed",
        message: `Erro de comunicação com Pague-X: ${err.message}`,
      };
    }
  }

  /**
   * Consulta o status de um pagamento na Pague-X.
   */
  async consultPayment(
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(
          `Erro ao consultar Pague-X (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`
        );
      }
    } catch (err: any) {
      throw new Error(`Falha ao consultar pagamento Pague-X: ${err.message}`);
    }
  }

  /**
   * Estorna/reembolsa um pagamento na Pague-X.
   */
  async refundPayment(
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Estorno Pague-X processado com sucesso.",
        };
      } else {
        return {
          success: false,
          amount: request.amount || 0,
          status: "failed",
          message: `Pague-X rejeitou o estorno (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
      return {
        ...
        amount: request.amount || 0,
        status: "failed",
        message: `Falha ao solicitar estorno na Pague-X: ${err.message}`,
      };
    }
  }

  /**
   * Processa webhook recebido da Pague-X.
   */
  async handleWebhook(
    payload: any,
    signature?: string,
