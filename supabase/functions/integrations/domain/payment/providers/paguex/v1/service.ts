...
export class Service extends BaseGateway {
  ...
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.ok || res.status === 200 || res.status === 404) {
        return { isValid: true, message: "Conexão estabelecida com Pague-X com sucesso." };
      } else {
        return { 
          isValid: false, 
          message: `Conexão rejeitada pelo Pague-X. HTTP status ${res.status}` 
        };
      }
    } catch (err: any) {
      return { isValid: false, message: `Erro ao conectar com Pague-X: ${err.message}` };
    }
  }
  ...
  override async createPix(request: PaymentRequest, config: IntegrationConfig): Promise<PaymentResponse> {
    ...
    const apiPayload = Mapper.toPaymentPayload(request, config.webhookUrl);
    ...
    try {
      ...
      if (res.ok) {
        return Mapper.toPaymentResponse(body, request.amount);
      } else {
        return {
          success: false,
          status: "failed",
          message: `Pague-X rejeitou o PIX: ${body.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Pague-X rejeitou o cartão: ${body.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Pague-X rejeitou o boleto: ${body.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
  ...
  override async consultPayment(gatewayTransactionId: string, config: IntegrationConfig): Promise<PaymentStatusResponse> {
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(`Erro ao consultar pagamento no Pague-X (${res.status}): ${body.message || "Erro desconhecido"}`);
      }
    } catch (err: any) {
