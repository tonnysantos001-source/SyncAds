...
export class Validator {
  static validateCredentials(credentials: any): { isValid: boolean; errors: string[] } {
    ...
    if (!credentials.merchantId) errors.push("merchantId is required");
    if (!credentials.merchantSiteId) errors.push("merchantSiteId is required");
    if (!credentials.secretKey) errors.push("secretKey is required");
