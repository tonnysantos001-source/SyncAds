...
export class Mapper {
  /**
   * Converte PaymentRequest no payload da Alpa.
   */
  static toCreatePaymentPayload(request: PaymentRequest, webhookUrl?: string): CreatePaymentPayload {
    ...
    // Alpa espera amount em centavos (padrão APIs brasileiras)
  ...
  /**
   * Converte resposta da Alpa para o padrão interno.
   */
  static toPaymentResponse(
    ...
    if (apiResponse.error || (!apiResponse.id && !apiResponse.transaction_id)) {
      return {
        success: false,
        status: "failed",
        message: apiResponse.error?.message || apiResponse.message || "Alpa recusou o pagamento.",
    ...
    const response: InternalPaymentResponse = {
      ...
      status: statusVal,
      message: apiResponse.message || `Alpa status: ${apiResponse.status}`,
      raw: apiResponse,
    };
  ...
  /**
   * Normaliza os códigos de status da Alpa.
   */
