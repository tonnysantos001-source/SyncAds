...
export class Service extends BaseGateway {
  readonly name = "Boleto Simples";
  readonly slug = "boletosimples";
  ...
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    return { isValid: true, message: "Credenciais Boleto Simples aceitas." };
  }
  ...
  async processPayment(request: PaymentRequest, config: IntegrationConfig): Promise<PaymentResponse> {
    ...
    const payload = Mapper.toBoletoPayload(request);
    try {
      const res = await client.createBoleto(payload);
      const body = await res.json();
      if (!res.ok) return { success: false, status: "failed", message: body?.message || `Boleto Simples erro ${res.status}`, raw: body };
      return Mapper.toPaymentResponse(body, request.orderId);
    } catch (e: any) {
      return { success: false, status: "failed", message: `Erro comunicação Boleto Simples: ${e.message}` };
    }
  }

  async consultPayment(gatewayTransactionId: string, config: IntegrationConfig): Promise<PaymentStatusResponse> {
    const client = this.getClient(config);
    const res = await client.getBoleto(gatewayTransactionId);
    const body = await res.json();
    if (!res.ok) throw new Error(`Boleto Simples erro ${res.status}`);
  ...
  async refundPayment(request: RefundRequest, config: IntegrationConfig): Promise<RefundResponse> {
    const client = this.getClient(config);
    try {
      const res = await client.cancelBoleto(request.gatewayTransactionId);
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Cancelamento solicitado no Boleto Simples.",
        };
      }
      return {
        success: false,
        amount: request.amount || 0,
        status: "failed",
        message: body?.message || `Erro Boleto Simples: ${res.status}`,
      };
    } catch (e: any) {
