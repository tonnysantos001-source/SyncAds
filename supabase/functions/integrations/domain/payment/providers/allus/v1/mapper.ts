...
export class Mapper {
  /**
   * Converte PaymentRequest no payload da Allus.
   */
  static toCreatePaymentPayload(request: PaymentRequest, webhookUrl?: string): CreatePaymentPayload {
    ...
    // Allus espera amount em centavos (padrão APIs brasileiras)
  ...
  /**
   * Converte resposta da Allus para o padrão interno.
   */
  static toPaymentResponse(
    ...
    if (apiResponse.error || (!apiResponse.id && !apiResponse.transaction_id)) {
      return {
        success: false,
        status: "failed",
        message: apiResponse.error?.message || apiResponse.message || "Allus recusou o pagamento.",
    ...
    const response: InternalPaymentResponse = {
      ...
      status: statusVal,
      message: apiResponse.message || `Allus status: ${apiResponse.status}`,
      raw: apiResponse,
    };

    if (apiResponse.qr_code || apiResponse.qr_code_base64) {
  ...
  /**
   * Normaliza os códigos de status da Allus.
   */
