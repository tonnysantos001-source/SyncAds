...
export class Service extends BaseGateway {
  readonly name = "Credwave";
  readonly slug = "credwave";
  ...
  /**
   * Valida as credenciais da Credwave.
   */
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          isValid: false,
          message: "Credenciais Credwave inválidas. Verifique o token.",
        };
      }

      return {
        isValid: true,
        message: "Credenciais Credwave validadas com sucesso.",
      };
    } catch (err: any) {
      return {
        isValid: true,
  ...
  /**
   * Processa pagamentos via Credwave.
   */
    ...
    try {
      ...
      if (!res.ok) {
        return {
          success: false,
          status: "failed",
          message: `Credwave rejeitou a transação (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
      ...
      return {
        success: false,
        status: "failed",
        message: `Erro de comunicação com Credwave: ${err.message}`,
      };
    }
  }

  /**
   * Consulta o status de um pagamento na Credwave.
   */
  async consultPayment(
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(
          `Erro ao consultar Credwave (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`
        );
      }
    } catch (err: any) {
      throw new Error(`Falha ao consultar pagamento Credwave: ${err.message}`);
    }
  }

  /**
   * Estorna/reembolsa um pagamento na Credwave.
   */
  async refundPayment(
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Estorno Credwave processado com sucesso.",
        };
      } else {
        return {
          success: false,
          amount: request.amount || 0,
          status: "failed",
          message: `Credwave rejeitou o estorno (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
      return {
        ...
        amount: request.amount || 0,
        status: "failed",
        message: `Falha ao solicitar estorno na Credwave: ${err.message}`,
      };
    }
  }

  /**
   * Processa webhook recebido da Credwave.
   */
  async handleWebhook(
    payload: any,
    signature?: string,
