...
export class Service extends BaseGateway {
  readonly name = "AnubisPay";
  readonly slug = "anubispay";
  ...
  /**
   * Valida as credenciais da AnubisPay.
   */
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          isValid: false,
          message: "Credenciais AnubisPay inválidas. Verifique o merchantId.",
        };
      }

      return {
        isValid: true,
        message: "Credenciais AnubisPay validadas com sucesso.",
      };
    } catch (err: any) {
      return {
        isValid: true,
  ...
  /**
   * Processa pagamentos via AnubisPay.
   */
    ...
    try {
      ...
      if (!res.ok) {
        return {
          success: false,
          status: "failed",
          message: `AnubisPay rejeitou a transação (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
      ...
      return {
        success: false,
        status: "failed",
        message: `Erro de comunicação com AnubisPay: ${err.message}`,
      };
    }
  }

  /**
   * Consulta o status de um pagamento na AnubisPay.
   */
  async consultPayment(
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(
          `Erro ao consultar AnubisPay (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`
        );
      }
    } catch (err: any) {
      throw new Error(`Falha ao consultar pagamento AnubisPay: ${err.message}`);
    }
  }

  /**
   * Estorna/reembolsa um pagamento na AnubisPay.
   */
  async refundPayment(
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Estorno AnubisPay processado com sucesso.",
        };
      } else {
        return {
          success: false,
          amount: request.amount || 0,
          status: "failed",
          message: `AnubisPay rejeitou o estorno (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
      return {
        ...
        amount: request.amount || 0,
        status: "failed",
        message: `Falha ao solicitar estorno na AnubisPay: ${err.message}`,
      };
    }
  }

  /**
   * Processa webhook recebido da AnubisPay.
   */
  async handleWebhook(
    payload: any,
    signature?: string,
