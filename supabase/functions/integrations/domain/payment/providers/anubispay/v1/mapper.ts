...
export class Mapper {
  /**
   * Converte PaymentRequest no payload da AnubisPay.
   */
  static toCreatePaymentPayload(request: PaymentRequest, webhookUrl?: string): CreatePaymentPayload {
    ...
    // AnubisPay espera amount em centavos (padrão APIs brasileiras)
  ...
  /**
   * Converte resposta da AnubisPay para o padrão interno.
   */
  static toPaymentResponse(
    ...
    if (apiResponse.error || (!apiResponse.id && !apiResponse.transaction_id)) {
      return {
        success: false,
        status: "failed",
        message: apiResponse.error?.message || apiResponse.message || "AnubisPay recusou o pagamento.",
    ...
    const response: InternalPaymentResponse = {
      ...
      status: statusVal,
      message: apiResponse.message || `AnubisPay status: ${apiResponse.status}`,
      raw: apiResponse,
    };
  ...
  /**
   * Normaliza os códigos de status da AnubisPay.
   */
