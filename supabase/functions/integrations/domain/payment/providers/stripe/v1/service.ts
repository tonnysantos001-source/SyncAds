...
export class Service extends BaseGateway {
  ...
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.ok || res.status === 200 || res.status === 404) {
        return { isValid: true, message: "Conexão estabelecida com Stripe com sucesso." };
      } else {
        return { 
          isValid: false, 
          message: `Conexão rejeitada pelo Stripe. HTTP status ${res.status}` 
        };
      }
    } catch (err: any) {
      return { isValid: false, message: `Erro ao conectar com Stripe: ${err.message}` };
    }
  }
  ...
  override async createPix(request: PaymentRequest, config: IntegrationConfig): Promise<PaymentResponse> {
    ...
    try {
      const res = await client.createPaymentIntent(apiPayload);
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Stripe rejeitou o PIX: ${body.error?.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Stripe rejeitou o cartão: ${body.error?.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Stripe rejeitou o boleto: ${body.error?.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
  ...
  override async consultPayment(gatewayTransactionId: string, config: IntegrationConfig): Promise<PaymentStatusResponse> {
    try {
      const client = this.getClient(config);
      const res = await client.getPaymentIntent(gatewayTransactionId);
      const body = await res.json();

      if (res.ok) {
        return Mapper.toPaymentStatusResponse(body);
      } else {
        throw new Error(`Erro ao consultar pagamento no Stripe (${res.status}): ${body.error?.message || "Erro desconhecido"}`);
      }
    } catch (err: any) {
