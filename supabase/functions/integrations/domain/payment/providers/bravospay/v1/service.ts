...
export class Service extends BaseGateway {
  readonly name = "Bravos Pay";
  readonly slug = "bravospay";
  ...
  /**
   * Valida as credenciais da Bravos Pay.
   */
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          isValid: false,
          message: "Credenciais Bravos Pay inválidas. Verifique a apiKey.",
        };
      }

      return {
        isValid: true,
        message: "Credenciais Bravos Pay validadas com sucesso.",
      };
    } catch (err: any) {
      return {
        isValid: true,
  ...
  /**
   * Processa pagamentos via Bravos Pay.
   */
    ...
    try {
      ...
      if (!res.ok) {
        return {
          success: false,
          status: "failed",
          message: `Bravos Pay rejeitou a transação (${res.status}): ${body?.error?.message || "Erro desconhecido"}`,
      ...
      return {
        success: false,
        status: "failed",
        message: `Erro de comunicação com Bravos Pay: ${err.message}`,
      };
    }
  }

  /**
   * Consulta o status de um pagamento na Bravos Pay.
   */
  async consultPayment(
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(
          `Erro ao consultar Bravos Pay (${res.status}): ${body?.error?.message || "Erro desconhecido"}`
        );
      }
    } catch (err: any) {
      throw new Error(`Falha ao consultar pagamento Bravos Pay: ${err.message}`);
    }
  }

  /**
   * Estorna/reembolsa um pagamento na Bravos Pay.
   */
  async refundPayment(
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Estorno Bravos Pay processado com sucesso.",
        };
      } else {
        return {
          success: false,
          amount: request.amount || 0,
          status: "failed",
          message: `Bravos Pay rejeitou o estorno (${res.status}): ${body?.error?.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
      return {
        ...
        amount: request.amount || 0,
        status: "failed",
        message: `Falha ao solicitar estorno na Bravos Pay: ${err.message}`,
      };
    }
  }

  /**
   * Processa webhook recebido da Bravos Pay.
   */
  async handleWebhook(
    payload: any,
    signature?: string,
