...
export class Validator {
  static validateCredentials(credentials: any): { isValid: boolean; errors: string[] } {
    ...
    if (!credentials.apiKey || credentials.apiKey.trim() === "") {
      errors.push("apiKey é obrigatória. Obtenha no painel do Dias Marketplace.");
    }
    if (!credentials.storeId || credentials.storeId.trim() === "") {
      errors.push("storeId é obrigatório. Obtenha no painel do Dias Marketplace.");
    }
