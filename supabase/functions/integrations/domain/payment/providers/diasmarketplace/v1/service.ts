...
export class Service extends BaseGateway {
  readonly name = "Diasmarketplace";
  readonly slug = "diasmarketplace";
  ...
  /**
   * Valida as credenciais do Dias Marketplace.
   */
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          isValid: false,
          message: "Credenciais Dias Marketplace inválidas. Verifique a apiKey.",
        };
      }

      return {
        isValid: true,
        message: "Credenciais Dias Marketplace validadas com sucesso.",
      };
    } catch (err: any) {
      return {
        isValid: true,
  ...
  /**
   * Processa pagamentos via Dias Marketplace.
   */
    ...
    try {
      ...
      if (!res.ok) {
        return {
          success: false,
          status: "failed",
          message: `Dias Marketplace rejeitou a transação (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
      ...
      return {
        success: false,
        status: "failed",
        message: `Erro de comunicação com Dias Marketplace: ${err.message}`,
      };
    }
  }

  /**
   * Consulta o status de um pagamento no Dias Marketplace.
   */
  async consultPayment(
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(
          `Erro ao consultar Dias Marketplace (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`
        );
      }
    } catch (err: any) {
      throw new Error(`Falha ao consultar pagamento Dias Marketplace: ${err.message}`);
    }
  }

  /**
   * Estorna/reembolsa um pagamento no Dias Marketplace.
   */
  async refundPayment(
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Estorno Dias Marketplace processado com sucesso.",
        };
      } else {
        return {
          success: false,
          amount: request.amount || 0,
          status: "failed",
          message: `Dias Marketplace rejeitou o estorno (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
      return {
        ...
        amount: request.amount || 0,
        status: "failed",
        message: `Falha ao solicitar estorno no Dias Marketplace: ${err.message}`,
      };
    }
  }

  /**
   * Processa webhook recebido do Dias Marketplace.
   */
  async handleWebhook(
    payload: any,
    signature?: string,
