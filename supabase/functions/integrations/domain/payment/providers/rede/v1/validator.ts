...
export class Validator {
  ...
  static validateCredentials(credentials: any): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
    if (!credentials.pv) {
      errors.push("PV (pv) é obrigatório.");
    }
    if (!credentials.token) {
      errors.push("Token de autenticação (token) é obrigatório.");
    }
    return {
  ...
  static validatePaymentRequest(request: PaymentRequest): { isValid: boolean; errors: string[] } {
    ...
    // Se for cartão de crédito ou débito, precisa de dados do cartão
    if (request.paymentMethod === "credit_card" || request.paymentMethod === "debit_card") {
      if (!request.card) {
        errors.push("Dados do cartão são obrigatórios para pagamento via cartão.");
      } else {
