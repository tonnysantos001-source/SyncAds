...
export class Service extends BaseGateway {
  readonly name = "FreePay";
  readonly slug = "freepay";
  ...
  /**
   * Valida as credenciais da Freepay.
   */
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          isValid: false,
          message: "Credenciais Freepay inválidas. Verifique o token.",
        };
      }

      return {
        isValid: true,
        message: "Credenciais Freepay validadas com sucesso.",
      };
    } catch (err: any) {
      return {
        isValid: true,
  ...
  /**
   * Processa pagamentos via Freepay.
   */
    ...
    try {
      ...
      if (!res.ok) {
        return {
          success: false,
          status: "failed",
          message: `Freepay rejeitou a transação (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
      ...
      return {
        success: false,
        status: "failed",
        message: `Erro de comunicação com Freepay: ${err.message}`,
      };
    }
  }

  /**
   * Consulta o status de um pagamento na Freepay.
   */
  async consultPayment(
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(
          `Erro ao consultar Freepay (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`
        );
      }
    } catch (err: any) {
      throw new Error(`Falha ao consultar pagamento Freepay: ${err.message}`);
    }
  }

  /**
   * Estorna/reembolsa um pagamento na Freepay.
   */
  async refundPayment(
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Estorno Freepay processado com sucesso.",
        };
      } else {
        return {
          success: false,
          amount: request.amount || 0,
          status: "failed",
          message: `Freepay rejeitou o estorno (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
      return {
        ...
        amount: request.amount || 0,
        status: "failed",
        message: `Falha ao solicitar estorno na Freepay: ${err.message}`,
      };
    }
  }

  /**
   * Processa webhook recebido da Freepay.
   */
  async handleWebhook(
    payload: any,
    signature?: string,
