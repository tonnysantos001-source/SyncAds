...
export class Validator {
  static validateCredentials(credentials: any): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
    if (!credentials.merchantCode || credentials.merchantCode.trim() === "") {
      errors.push("merchantCode é obrigatório. Obtenha no painel da Aston Pay.");
    }
    if (!credentials.apiKey || credentials.apiKey.trim() === "") {
      errors.push("apiKey (Secret Key) é obrigatória. Obtenha no painel da Aston Pay.");
    }
