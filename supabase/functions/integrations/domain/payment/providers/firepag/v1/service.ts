...
export class Service extends BaseGateway {
  readonly name = "Fire Pag";
  readonly slug = "firepag";
  ...
  /**
   * Valida as credenciais da Firepag.
   */
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          isValid: false,
          message: "Credenciais Firepag inválidas. Verifique o token.",
        };
      }

      return {
        isValid: true,
        message: "Credenciais Firepag validadas com sucesso.",
      };
    } catch (err: any) {
      return {
        isValid: true,
  ...
  /**
   * Processa pagamentos via Firepag.
   */
    ...
    try {
      ...
      if (!res.ok) {
        return {
          success: false,
          status: "failed",
          message: `Firepag rejeitou a transação (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
      ...
      return {
        success: false,
        status: "failed",
        message: `Erro de comunicação com Firepag: ${err.message}`,
      };
    }
  }

  /**
   * Consulta o status de um pagamento na Firepag.
   */
  async consultPayment(
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(
          `Erro ao consultar Firepag (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`
        );
      }
    } catch (err: any) {
      throw new Error(`Falha ao consultar pagamento Firepag: ${err.message}`);
    }
  }

  /**
   * Estorna/reembolsa um pagamento na Firepag.
   */
  async refundPayment(
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Estorno Firepag processado com sucesso.",
        };
      } else {
        return {
          success: false,
          amount: request.amount || 0,
          status: "failed",
          message: `Firepag rejeitou o estorno (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
      return {
        ...
        amount: request.amount || 0,
        status: "failed",
        message: `Falha ao solicitar estorno na Firepag: ${err.message}`,
      };
    }
  }

  /**
   * Processa webhook recebido da Firepag.
   */
  async handleWebhook(
    payload: any,
    signature?: string,
