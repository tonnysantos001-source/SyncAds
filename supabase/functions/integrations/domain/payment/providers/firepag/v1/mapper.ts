...
export class Mapper {
  /**
   * Converte PaymentRequest no payload da Firepag.
   */
  static toCreatePaymentPayload(request: PaymentRequest, webhookUrl?: string): CreatePaymentPayload {
    ...
    // Firepag espera amount em centavos (padrão APIs brasileiras)
  ...
  /**
   * Converte resposta da Firepag para o padrão interno.
   */
  static toPaymentResponse(
    ...
    if (apiResponse.error || (!apiResponse.id && !apiResponse.transaction_id)) {
      return {
        success: false,
        status: "failed",
        message: apiResponse.error?.message || apiResponse.message || "Firepag recusou o pagamento.",
    ...
    const response: InternalPaymentResponse = {
      ...
      status: statusVal,
      message: apiResponse.message || `Firepag status: ${apiResponse.status}`,
      raw: apiResponse,
    };
  ...
  /**
   * Normaliza os códigos de status da Firepag.
   */
