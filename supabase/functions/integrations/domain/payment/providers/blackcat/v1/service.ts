  ...
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.ok || res.status === 404) {
        return { isValid: true, message: "Conexão com Blackcat validada com sucesso." };
      } else {
        const body = await res.json().catch(() => ({}));
        return { 
          isValid: false, 
          message: `Conexão rejeitada pela Blackcat. HTTP status ${res.status}: ${body.message || "Erro desconhecido"}` 
        };
      }
    } catch (err: any) {
      return { isValid: false, message: `Erro de rede ao conectar com Blackcat: ${err.message}` };
    }
  }
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Blackcat rejeitou a cobrança Pix (${res.status}): ${body.message || "Erro desconhecido"}`,
          errorCode: body.error || "PIX_ERROR",
        };
      }
    } catch (err: any) {
      return { success: false, status: "failed", message: `Erro de comunicação com Blackcat: ${err.message}` };
    }
  }
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Blackcat rejeitou o pagamento com cartão (${res.status}): ${body.message || "Erro desconhecido"}`,
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Blackcat rejeitou a emissão do boleto (${res.status}): ${body.message || "Erro desconhecido"}`,
  ...
  async consultPayment(gatewayTransactionId: string, config: IntegrationConfig): Promise<PaymentStatusResponse> {
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(`Erro ao consultar pagamento na Blackcat (${res.status}): ${body.message}`);
      }
    } catch (err: any) {
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Reembolso processado com sucesso na Blackcat.",
        };
      } else {
        return {
          success: false,
          ...
          status: "failed",
          message: `Erro ao reembolsar transação na Blackcat (${res.status}): ${body.message}`,
        };
      }
    } catch (err: any) {
      return {
