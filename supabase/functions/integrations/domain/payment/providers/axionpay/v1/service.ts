...
export class Service extends BaseGateway {
  readonly name = "Axion Pay";
  readonly slug = "axionpay";
  ...
  /**
   * Valida as credenciais da Axion Pay.
   */
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          isValid: false,
          message: "Credenciais Axion Pay inválidas. Verifique a apiKey.",
        };
      }

      return {
        isValid: true,
        message: "Credenciais Axion Pay validadas com sucesso.",
      };
    } catch (err: any) {
      return {
        isValid: true,
  ...
  /**
   * Processa pagamentos via Axion Pay.
   */
    ...
    try {
      ...
      if (!res.ok) {
        return {
          success: false,
          status: "failed",
          message: `Axion Pay rejeitou a transação (${res.status}): ${body?.error?.message || "Erro desconhecido"}`,
      ...
      return {
        success: false,
        status: "failed",
        message: `Erro de comunicação com Axion Pay: ${err.message}`,
      };
    }
  }

  /**
   * Consulta o status de um pagamento na Axion Pay.
   */
  async consultPayment(
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(
          `Erro ao consultar Axion Pay (${res.status}): ${body?.error?.message || "Erro desconhecido"}`
        );
      }
    } catch (err: any) {
      throw new Error(`Falha ao consultar pagamento Axion Pay: ${err.message}`);
    }
  }

  /**
   * Estorna/reembolsa um pagamento na Axion Pay.
   */
  async refundPayment(
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Estorno Axion Pay processado com sucesso.",
        };
      } else {
        return {
          success: false,
          amount: request.amount || 0,
          status: "failed",
          message: `Axion Pay rejeitou o estorno (${res.status}): ${body?.error?.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
      return {
        ...
        amount: request.amount || 0,
        status: "failed",
        message: `Falha ao solicitar estorno na Axion Pay: ${err.message}`,
      };
    }
  }

  /**
   * Processa webhook recebido da Axion Pay.
   */
  async handleWebhook(
    payload: any,
    signature?: string,
