...
export class Mapper {
  /**
   * Converte PaymentRequest no payload da Axion Pay.
   */
  static toCreateChargePayload(request: PaymentRequest, webhookUrl?: string): CreateChargePayload {
    ...
    let method: "credit_card" | "pix" | "boleto" = "credit_card";
    if (request.paymentMethod === "pix") method = "pix";
    else if (request.paymentMethod === "boleto") method = "boleto";
    ...
    if (request.card && method === "credit_card") {
      payload.card = {
  ...
  /**
   * Converte resposta da Axion Pay para o padrão interno.
   */
  static toPaymentResponse(
    ...
    if (apiResponse.error || !apiResponse.id) {
      return {
        success: false,
        status: "failed",
        message: apiResponse.error?.message || "Axion Pay recusou a transação.",
    ...
    const response: InternalPaymentResponse = {
      ...
      message: `Axion Pay status: ${apiResponse.status}`,
      raw: apiResponse,
    };
  ...
  static toPaymentStatusResponse(apiResponse: ChargeResponse): PaymentStatusResponse {
    return {
      ...
      amount: (apiResponse.amount || 0) / 100,
      currency: "BRL",
      paymentMethod: apiResponse.method || "unknown",
  ...
  /**
   * Normaliza os códigos de status da Axion Pay.
   */
