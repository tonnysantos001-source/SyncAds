...
export class Service extends BaseGateway {
  ...
  /**
   * Valida as credenciais da Asset.
   */
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          isValid: false,
          message: "Credenciais Asset inválidas. Verifique a publicKey.",
        };
      }

      return {
        isValid: true,
        message: "Credenciais Asset validadas com sucesso.",
      };
    } catch (err: any) {
      return {
        isValid: true,
    ...
    try {
      ...
      if (!res.ok) {
        return {
          success: false,
          status: "failed",
          message: `Asset rejeitou a transação (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
      ...
      return {
        success: false,
        status: "failed",
        message: `Erro de comunicação com Asset: ${err.message}`,
      };
    }
  }

  /**
   * Consulta o status de um pagamento na Asset.
   */
  async consultPayment(
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(
          `Erro ao consultar Asset (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`
        );
      }
    } catch (err: any) {
      throw new Error(`Falha ao consultar pagamento Asset: ${err.message}`);
    }
  }

  /**
   * Estorna/reembolsa um pagamento na Asset.
   */
  async refundPayment(
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Estorno Asset processado com sucesso.",
        };
      } else {
        return {
          success: false,
          amount: request.amount || 0,
          status: "failed",
          message: `Asset rejeitou o estorno (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
      return {
        ...
        amount: request.amount || 0,
        status: "failed",
        message: `Falha ao solicitar estorno na Asset: ${err.message}`,
      };
    }
  }

  /**
   * Processa webhook recebido da Asset.
   */
  async handleWebhook(
    payload: any,
    signature?: string,
