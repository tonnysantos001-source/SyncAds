...
export class Service extends BaseGateway {
  readonly name = "Pagou.ai";
  readonly slug = "pagouai";
  ...
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.ok) {
        return { isValid: true, message: "Conexão com Pagou.ai validada com sucesso." };
      } else {
        const body = await res.json().catch(() => ({}));
        return { 
          isValid: false, 
          message: `Conexão rejeitada pelo Pagou.ai. HTTP status ${res.status}: ${body.message || "Erro desconhecido"}` 
        };
      }
    } catch (err: any) {
      return { isValid: false, message: `Erro de rede ao conectar com Pagou.ai: ${err.message}` };
    }
  }
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Pagou.ai rejeitou a cobrança Pix (${res.status}): ${body.message || "Erro desconhecido"}`,
          errorCode: body.error || "PIX_ERROR",
        };
      }
    } catch (err: any) {
      return { success: false, status: "failed", message: `Erro de comunicação com Pagou.ai: ${err.message}` };
    }
  }
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Pagou.ai rejeitou o pagamento com cartão (${res.status}): ${body.message || "Erro desconhecido"}`,
  ...
  async createBoleto(request: PaymentRequest, config: IntegrationConfig): Promise<PaymentResponse> {
    ...
    // Forçar voucher no payload
    apiPayload.method = "voucher";
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Pagou.ai rejeitou a emissão do boleto (${res.status}): ${body.message || "Erro desconhecido"}`,
  ...
  async consultPayment(gatewayTransactionId: string, config: IntegrationConfig): Promise<PaymentStatusResponse> {
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(`Erro ao consultar pagamento no Pagou.ai (${res.status}): ${body.message}`);
      }
    } catch (err: any) {
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Reembolso processado com sucesso no Pagou.ai.",
        };
      } else {
        return {
          success: false,
          ...
          status: "failed",
          message: `Erro ao reembolsar transação no Pagou.ai (${res.status}): ${body.message}`,
        };
      }
    } catch (err: any) {
      return {
        ...
        amount: request.amount || 0,
        status: "failed",
        message: `Falha ao solicitar estorno no Pagou.ai: ${err.message}`,
      };
    }
  }
