...
export class Service extends BaseGateway {
  readonly name = "SafetyPay";
  readonly slug = "safetypay";
  ...
  /**
   * Valida as credenciais do SafetyPay.
   */
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          isValid: false,
          message: "Credenciais SafetyPay inválidas. Verifique a apiKey.",
        };
      }

      return {
        isValid: true,
        message: "Credenciais SafetyPay validadas com sucesso.",
      };
    } catch (err: any) {
      return {
        isValid: true,
  ...
  /**
   * Processa pagamentos via SafetyPay.
   */
    ...
    try {
      ...
      if (!res.ok) {
        return {
          success: false,
          status: "failed",
          message: `SafetyPay rejeitou a transação (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
      ...
      return {
        success: false,
        status: "failed",
        message: `Erro de comunicação com SafetyPay: ${err.message}`,
      };
    }
  }

  /**
   * Consulta o status de um pagamento na SafetyPay.
   */
  async consultPayment(
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(
          `Erro ao consultar SafetyPay (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`
        );
      }
    } catch (err: any) {
      throw new Error(`Falha ao consultar pagamento SafetyPay: ${err.message}`);
    }
  }

  /**
   * Estorna/reembolsa um pagamento na SafetyPay.
   */
  async refundPayment(
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Estorno SafetyPay processado com sucesso.",
        };
      } else {
        return {
          success: false,
          amount: request.amount || 0,
          status: "failed",
          message: `SafetyPay rejeitou o estorno (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
      return {
        ...
        amount: request.amount || 0,
        status: "failed",
        message: `Falha ao solicitar estorno na SafetyPay: ${err.message}`,
      };
    }
  }

  /**
   * Processa webhook recebido da SafetyPay.
   */
  async handleWebhook(
    payload: any,
    signature?: string,
