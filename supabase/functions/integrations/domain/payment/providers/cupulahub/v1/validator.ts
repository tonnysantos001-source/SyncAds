...
export class Validator {
  static validateCredentials(credentials: any): { isValid: boolean; errors: string[] } {
    ...
    if (!credentials.apiKey || credentials.apiKey.trim() === "") {
      errors.push("apiKey é obrigatória. Obtenha no painel da Cúpula Hub.");
    }
    if (!credentials.hubId || credentials.hubId.trim() === "") {
      errors.push("hubId é obrigatório. Obtenha no painel da Cúpula Hub.");
    }
