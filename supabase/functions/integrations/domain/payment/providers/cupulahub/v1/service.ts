...
export class Service extends BaseGateway {
  readonly name = "Cúpula Hub";
  readonly slug = "cupulahub";
  ...
  /**
   * Valida as credenciais da Cúpula Hub.
   */
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          isValid: false,
          message: "Credenciais Cúpula Hub inválidas. Verifique a apiKey.",
        };
      }

      return {
        isValid: true,
        message: "Credenciais Cúpula Hub validadas com sucesso.",
      };
    } catch (err: any) {
      return {
        isValid: true,
  ...
  /**
   * Processa pagamentos via Cúpula Hub.
   */
    ...
    try {
      ...
      if (!res.ok) {
        return {
          success: false,
          status: "failed",
          message: `Cúpula Hub rejeitou a transação (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
      ...
      return {
        success: false,
        status: "failed",
        message: `Erro de comunicação com Cúpula Hub: ${err.message}`,
      };
    }
  }

  /**
   * Consulta o status de um pagamento na Cúpula Hub.
   */
  async consultPayment(
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(
          `Erro ao consultar Cúpula Hub (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`
        );
      }
    } catch (err: any) {
      throw new Error(`Falha ao consultar pagamento Cúpula Hub: ${err.message}`);
    }
  }

  /**
   * Estorna/reembolsa um pagamento na Cúpula Hub.
   */
  async refundPayment(
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Estorno Cúpula Hub processado com sucesso.",
        };
      } else {
        return {
          success: false,
          amount: request.amount || 0,
          status: "failed",
          message: `Cúpula Hub rejeitou o estorno (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
      return {
        ...
        amount: request.amount || 0,
        status: "failed",
        message: `Falha ao solicitar estorno na Cúpula Hub: ${err.message}`,
      };
    }
  }

  /**
   * Processa webhook recebido da Cúpula Hub.
   */
  async handleWebhook(
    payload: any,
    signature?: string,
