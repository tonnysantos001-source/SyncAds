...
export class Service extends BaseGateway {
  ...
  /**
   * Valida as credenciais do Dom Pagamentos.
   */
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          isValid: false,
          message: "Credenciais Dom Pagamentos inválidas. Verifique o token.",
        };
      }

      return {
        isValid: true,
        message: "Credenciais Dom Pagamentos validadas com sucesso.",
      };
    } catch (err: any) {
      return {
        isValid: true,
  ...
  /**
   * Processa pagamentos via Dom Pagamentos.
   */
    ...
    try {
      ...
      if (!res.ok) {
        return {
          success: false,
          status: "failed",
          message: `Dom Pagamentos rejeitou a transação (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
      ...
      return {
        success: false,
        status: "failed",
        message: `Erro de comunicação com Dom Pagamentos: ${err.message}`,
      };
    }
  }

  /**
   * Consulta o status de um pagamento no Dom Pagamentos.
   */
  async consultPayment(
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(
          `Erro ao consultar Dom Pagamentos (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`
        );
      }
    } catch (err: any) {
      throw new Error(`Falha ao consultar pagamento Dom Pagamentos: ${err.message}`);
    }
  }

  /**
   * Estorna/reembolsa um pagamento no Dom Pagamentos.
   */
  async refundPayment(
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Estorno Dom Pagamentos processado com sucesso.",
        };
      } else {
        return {
          success: false,
          amount: request.amount || 0,
          status: "failed",
          message: `Dom Pagamentos rejeitou o estorno (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
      return {
        ...
        amount: request.amount || 0,
        status: "failed",
        message: `Falha ao solicitar estorno no Dom Pagamentos: ${err.message}`,
      };
    }
  }

  /**
   * Processa webhook recebido do Dom Pagamentos.
   */
  async handleWebhook(
    payload: any,
    signature?: string,
