...
export class Service extends BaseGateway {
  readonly name = "Dom Pagamentos";
  readonly slug = "dompagamentos";
  ...
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.ok) {
        return { isValid: true, message: "Conexão com Dom Pagamentos validada com sucesso." };
      } else {
        const body = await res.json().catch(() => ({}));
        return { 
          isValid: false, 
          message: `Conexão rejeitada pela Dom Pagamentos. HTTP status ${res.status}: ${body.message || "Erro desconhecido"}` 
        };
      }
    } catch (err: any) {
      return { isValid: false, message: `Erro de rede ao conectar com Dom Pagamentos: ${err.message}` };
    }
  }
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Dom Pagamentos rejeitou a cobrança Pix (${res.status}): ${body.message || "Erro desconhecido"}`,
          errorCode: body.error || "PIX_ERROR",
        };
      }
    } catch (err: any) {
      return { success: false, status: "failed", message: `Erro de comunicação com Dom Pagamentos: ${err.message}` };
    }
  }
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Dom Pagamentos rejeitou o pagamento com cartão (${res.status}): ${body.message || "Erro desconhecido"}`,
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Dom Pagamentos rejeitou a emissão do boleto (${res.status}): ${body.message || "Erro desconhecido"}`,
  ...
  async consultPayment(gatewayTransactionId: string, config: IntegrationConfig): Promise<PaymentStatusResponse> {
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(`Erro ao consultar pagamento na Dom Pagamentos (${res.status}): ${body.message}`);
      }
    } catch (err: any) {
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Reembolso processado com sucesso na Dom Pagamentos.",
        };
      } else {
        return {
          success: false,
          ...
          status: "failed",
          message: `Erro ao reembolsar transação na Dom Pagamentos (${res.status}): ${body.message}`,
        };
      }
    } catch (err: any) {
      return {
        ...
        amount: request.amount || 0,
        status: "failed",
        message: `Falha ao solicitar estorno na Dom Pagamentos: ${err.message}`,
      };
    }
  }
