...
export class Service extends BaseGateway {
  ...
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.ok || res.status === 200 || res.status === 404) {
        return { isValid: true, message: "Conexão estabelecida com Cyberhub com sucesso." };
      } else {
        return { 
          isValid: false, 
          message: `Conexão rejeitada pelo Cyberhub. HTTP status ${res.status}` 
        };
      }
    } catch (err: any) {
      return { isValid: false, message: `Erro ao conectar com Cyberhub: ${err.message}` };
    }
  }
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Cyberhub rejeitou o PIX: ${body.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Cyberhub rejeitou o cartão: ${body.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
  ...
  /**
   * Processamento de Boleto (Cyberhub não suporta Boleto, retorna falha)
   */
  override async createBoleto(_request: PaymentRequest, _config: IntegrationConfig): Promise<PaymentResponse> {
    return { success: false, status: "failed", message: "Cyberhub não suporta boleto." };
  }
  ...
  override async consultPayment(gatewayTransactionId: string, config: IntegrationConfig): Promise<PaymentStatusResponse> {
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(`Erro ao consultar pagamento no Cyberhub (${res.status}): ${body.message || "Erro desconhecido"}`);
      }
    } catch (err: any) {
