...
export class Mapper {
  /**
   * Converte PaymentRequest no payload da Cyberhub.
   */
  static toCreatePaymentPayload(request: PaymentRequest, webhookUrl?: string): CreatePaymentPayload {
    ...
    let payment_method: "credit_card" | "pix" | "boleto" | "wallet" = "credit_card";
    if (request.paymentMethod === "pix") payment_method = "pix";
    else if (request.paymentMethod === "boleto") payment_method = "boleto";

    // Cyberhub espera amount em centavos (padrão APIs brasileiras)
  ...
  /**
   * Converte resposta da Cyberhub para o padrão interno.
   */
  static toPaymentResponse(
    ...
    if (apiResponse.error || (!apiResponse.id && !apiResponse.transaction_id)) {
      return {
        success: false,
        status: "failed",
        message: apiResponse.error?.message || apiResponse.message || "Cyberhub recusou o pagamento.",
    ...
    const response: InternalPaymentResponse = {
      ...
      status: statusVal,
      message: apiResponse.message || `Cyberhub status: ${apiResponse.status}`,
      raw: apiResponse,
    };
  ...
  /**
   * Normaliza os códigos de status da Cyberhub.
   */
