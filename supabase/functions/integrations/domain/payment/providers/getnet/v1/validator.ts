...
export class Validator {
  static validateCredentials(credentials: any): { isValid: boolean; errors: string[] } {
    ...
    if (!credentials.clientId || credentials.clientId.trim() === "") {
      errors.push("clientId é obrigatório. Obtenha no painel da GetNet.");
    }
    if (!credentials.clientSecret || credentials.clientSecret.trim() === "") {
      errors.push("clientSecret é obrigatório. Obtenha no painel da GetNet.");
    }
    if (!credentials.sellerId || credentials.sellerId.trim() === "") {
      errors.push("sellerId é obrigatório. Obtenha no painel da GetNet.");
    }
