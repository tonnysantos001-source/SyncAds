...
export class Service extends BaseGateway {
  readonly name = "Fivepay";
  readonly slug = "fivepay";
  ...
  /**
   * Valida as credenciais da Fivepay.
   */
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          isValid: false,
          message: "Credenciais Fivepay inválidas. Verifique a apiKey.",
        };
      }

      return {
        isValid: true,
        message: "Credenciais Fivepay validadas com sucesso.",
      };
    } catch (err: any) {
      return {
        isValid: true,
  ...
  /**
   * Processa pagamentos via Fivepay.
   */
    ...
    try {
      ...
      if (!res.ok) {
        return {
          success: false,
          status: "failed",
          message: `Fivepay rejeitou a transação (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
      ...
      return {
        success: false,
        status: "failed",
        message: `Erro de comunicação com Fivepay: ${err.message}`,
      };
    }
  }

  /**
   * Consulta o status de um pagamento na Fivepay.
   */
  async consultPayment(
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(
          `Erro ao consultar Fivepay (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`
        );
      }
    } catch (err: any) {
      throw new Error(`Falha ao consultar pagamento Fivepay: ${err.message}`);
    }
  }

  /**
   * Estorna/reembolsa um pagamento na Fivepay.
   */
  async refundPayment(
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Estorno Fivepay processado com sucesso.",
        };
      } else {
        return {
          success: false,
          amount: request.amount || 0,
          status: "failed",
          message: `Fivepay rejeitou o estorno (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
      return {
        ...
        amount: request.amount || 0,
        status: "failed",
        message: `Falha ao solicitar estorno na Fivepay: ${err.message}`,
      };
    }
  }

  /**
   * Processa webhook recebido da Fivepay.
   */
  async handleWebhook(
    payload: any,
    signature?: string,
