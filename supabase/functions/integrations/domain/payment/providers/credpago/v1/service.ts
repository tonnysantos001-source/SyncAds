...
export class Service extends BaseGateway {
  readonly name = "Credpago";
  readonly slug = "credpago";
  ...
  /**
   * Valida as credenciais da Credpago.
   */
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          isValid: false,
          message: "Credenciais Credpago inválidas. Verifique a apiKey.",
        };
      }

      return {
        isValid: true,
        message: "Credenciais Credpago validadas com sucesso.",
      };
    } catch (err: any) {
      return {
        isValid: true,
  ...
  /**
   * Processa pagamentos via Credpago.
   */
    ...
    try {
      ...
      if (!res.ok) {
        return {
          success: false,
          status: "failed",
          message: `Credpago rejeitou a transação (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
      ...
      return {
        success: false,
        status: "failed",
        message: `Erro de comunicação com Credpago: ${err.message}`,
      };
    }
  }

  /**
   * Consulta o status de um pagamento na Credpago.
   */
  async consultPayment(
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(
          `Erro ao consultar Credpago (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`
        );
      }
    } catch (err: any) {
      throw new Error(`Falha ao consultar pagamento Credpago: ${err.message}`);
    }
  }

  /**
   * Estorna/reembolsa um pagamento na Credpago.
   */
  async refundPayment(
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Estorno Credpago processado com sucesso.",
        };
      } else {
        return {
          success: false,
          amount: request.amount || 0,
          status: "failed",
          message: `Credpago rejeitou o estorno (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
      return {
        ...
        amount: request.amount || 0,
        status: "failed",
        message: `Falha ao solicitar estorno na Credpago: ${err.message}`,
      };
    }
  }

  /**
   * Processa webhook recebido da Credpago.
   */
  async handleWebhook(
    payload: any,
    signature?: string,
