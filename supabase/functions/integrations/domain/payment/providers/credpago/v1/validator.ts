...
export class Validator {
  static validateCredentials(credentials: any): { isValid: boolean; errors: string[] } {
    ...
    if (!credentials.apiKey || credentials.apiKey.trim() === "") {
      errors.push("apiKey é obrigatória. Obtenha no painel da Credpago.");
    }
    if (!credentials.partnerId || credentials.partnerId.trim() === "") {
      errors.push("partnerId é obrigatório. Obtenha no painel da Credpago.");
    }
