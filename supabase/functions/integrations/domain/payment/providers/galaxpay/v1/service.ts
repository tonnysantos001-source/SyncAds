...
export class Service extends BaseGateway {
  readonly name = "GalaxPay";
  readonly slug = "galaxpay";
  ...
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    return { isValid: true, message: "Credenciais GalaxPay aceitas." };
  }
  ...
  async processPayment(request: PaymentRequest, config: IntegrationConfig): Promise<PaymentResponse> {
    ...
    try {
      ...
      if (!res.ok) return { success: false, status: "failed", message: body?.error?.message || `GalaxPay erro ${res.status}`, raw: body };
      return Mapper.toPaymentResponse(body, request.orderId);
    } catch (e: any) {
      return { success: false, status: "failed", message: `Erro comunicação GalaxPay: ${e.message}` };
    }
  }
  ...
  async consultPayment(gatewayTransactionId: string, config: IntegrationConfig): Promise<PaymentStatusResponse> {
    ...
    const body = await res.json();
    if (!res.ok) throw new Error(`GalaxPay erro ${res.status}`);
    // Se retornar lista de transações, pega a primeira
    const transaction = Array.isArray(body?.Transactions) ? body.Transactions[0] : body;
    return Mapper.toPaymentStatusResponse(transaction);
  }
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Estorno GalaxPay solicitado.",
        };
      }
      return {
        success: false,
        amount: request.amount || 0,
        status: "failed",
        message: body?.error?.message || `Erro GalaxPay: ${res.status}`,
      };
    } catch (e: any) {
