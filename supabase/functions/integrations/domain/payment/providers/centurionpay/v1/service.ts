  ...
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.ok || res.status === 200 || res.status === 404) {
        return { isValid: true, message: "Conexão estabelecida com Centurion Pay com sucesso." };
      } else {
        return { 
          isValid: false, 
          message: `Conexão rejeitada pelo Centurion Pay. HTTP status ${res.status}` 
        };
      }
    } catch (err: any) {
      return { isValid: false, message: `Erro ao conectar com Centurion Pay: ${err.message}` };
    }
  }
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Centurion Pay rejeitou o PIX: ${body.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Centurion Pay rejeitou o cartão: ${body.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Centurion Pay rejeitou o boleto: ${body.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
  ...
  override async consultPayment(gatewayTransactionId: string, config: IntegrationConfig): Promise<PaymentStatusResponse> {
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(`Erro ao consultar pagamento no Centurion Pay (${res.status}): ${body.message || "Erro desconhecido"}`);
      }
    } catch (err: any) {
