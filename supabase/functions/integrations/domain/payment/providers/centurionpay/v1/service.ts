...
export class Service extends BaseGateway {
  readonly name = "Centurion Pay";
  readonly slug = "centurionpay";
  ...
  /**
   * Valida as credenciais da Centurion Pay.
   */
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          isValid: false,
          message: "Credenciais Centurion Pay inválidas. Verifique o apiToken e merchantId.",
        };
      }

      return {
        isValid: true,
        message: "Credenciais Centurion Pay validadas com sucesso.",
      };
    } catch (err: any) {
      return {
        isValid: true,
  ...
  /**
   * Processa pagamentos via Centurion Pay.
   */
    ...
    try {
      ...
      if (!res.ok) {
        return {
          success: false,
          status: "failed",
          message: `Centurion Pay rejeitou a transação (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
      ...
      return {
        success: false,
        status: "failed",
        message: `Erro de comunicação com Centurion Pay: ${err.message}`,
      };
    }
  }

  /**
   * Consulta o status de um pagamento na Centurion Pay.
   */
  async consultPayment(
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(
          `Erro ao consultar Centurion Pay (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`
        );
      }
    } catch (err: any) {
      throw new Error(`Falha ao consultar pagamento Centurion Pay: ${err.message}`);
    }
  }

  /**
   * Estorna/reembolsa um pagamento na Centurion Pay.
   */
  async refundPayment(
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Estorno Centurion Pay processado com sucesso.",
        };
      } else {
        return {
          success: false,
          amount: request.amount || 0,
          status: "failed",
          message: `Centurion Pay rejeitou o estorno (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
      return {
        ...
        amount: request.amount || 0,
        status: "failed",
        message: `Falha ao solicitar estorno na Centurion Pay: ${err.message}`,
      };
    }
  }

  /**
   * Processa webhook recebido da Centurion Pay.
   */
  async handleWebhook(
    payload: any,
    signature?: string,
