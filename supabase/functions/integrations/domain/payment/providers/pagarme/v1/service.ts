...
export class Service extends BaseGateway {
  ...
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.ok) {
        return { isValid: true, message: "Conexão estabelecida com Pagar.me com sucesso." };
      } else {
        const body = await res.json().catch(() => ({}));
        return { 
          isValid: false, 
          message: `Conexão rejeitada pelo Pagar.me. HTTP status ${res.status}: ${body.message || "Erro de autenticação"}` 
        };
      }
    } catch (err: any) {
      return { isValid: false, message: `Erro de rede ao conectar com Pagar.me: ${err.message}` };
    }
  }
  ...
  async createPix(request: PaymentRequest, config: IntegrationConfig): Promise<PaymentResponse> {
    ...
    try {
      const res = await client.createOrder(apiPayload);
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Pagar.me rejeitou o PIX: ${body.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Pagar.me rejeitou o cartão: ${body.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Pagar.me rejeitou o boleto: ${body.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
  ...
  async consultPayment(gatewayTransactionId: string, config: IntegrationConfig): Promise<PaymentStatusResponse> {
    try {
      const client = this.getClient(config);
      // gatewayTransactionId no Pagar.me é o ID da Order ou Charge
      ...
      if (res.ok) {
        ...
        throw new Error(`Erro ao consultar pagamento no Pagar.me (${res.status}): ${body.message || "Erro desconhecido"}`);
      }
    } catch (err: any) {
