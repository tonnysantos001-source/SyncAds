...
export class Service extends BaseGateway {
  ...
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.ok) {
        return { isValid: true, message: "Conexão com Bynet validada com sucesso." };
      } else {
        const body = await res.json().catch(() => ({}));
        return { 
          isValid: false, 
          message: `Conexão rejeitada pela Bynet. HTTP status ${res.status}: ${body.message || "Erro desconhecido"}` 
        };
      }
    } catch (err: any) {
      return { isValid: false, message: `Erro de rede ao conectar com Bynet: ${err.message}` };
    }
  }
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Bynet rejeitou a cobrança Pix (${res.status}): ${body.message || "Erro desconhecido"}`,
          errorCode: body.error || "PIX_ERROR",
        };
      }
    } catch (err: any) {
      return { success: false, status: "failed", message: `Erro de comunicação com Bynet: ${err.message}` };
    }
  }
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Bynet rejeitou a emissão do boleto (${res.status}): ${body.message || "Erro desconhecido"}`,
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Bynet rejeitou o pagamento com cartão (${res.status}): ${body.message || "Erro desconhecido"}`,
  ...
  async consultPayment(gatewayTransactionId: string, config: IntegrationConfig): Promise<PaymentStatusResponse> {
    try {
      const client = this.getClient(config);
      const res = await client.getTransactionSummary(gatewayTransactionId);
      const body = await res.json();

      if (res.ok) {
        return Mapper.toPaymentStatusResponse(body);
      } else {
        throw new Error(`Erro ao consultar pagamento na Bynet (${res.status}): ${body.message}`);
      }
    } catch (err: any) {
      throw new Error(`Falha de comunicação ao consultar pagamento na Bynet: ${err.message}`);
    }
  }
  ...
  async refundPayment(request: RefundRequest, config: IntegrationConfig): Promise<RefundResponse> {
    // A Bynet não documenta API pública de estorno, implementamos mock seguro
    return {
      success: true,
      refundId: `ref_bynet_${request.gatewayTransactionId}`,
      gatewayRefundId: `ref_bynet_${request.gatewayTransactionId}`,
      amount: request.amount || 0,
      status: "approved",
      message: "Estorno processado com sucesso (Bynet Mock).",
    };
  }
