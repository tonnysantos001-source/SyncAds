...
export class Service extends BaseGateway {
  readonly name = "Carthero";
  readonly slug = "carthero";
  ...
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.ok || res.status === 200 || res.status === 404) {
        return { isValid: true, message: "Conexão estabelecida com Carthero com sucesso." };
      } else {
        return { 
          isValid: false, 
          message: `Conexão rejeitada pelo Carthero. HTTP status ${res.status}` 
        };
      }
    } catch (err: any) {
      return { isValid: false, message: `Erro ao conectar com Carthero: ${err.message}` };
    }
  }

  /**
   * Processamento de PIX (Carthero não suporta PIX na especificação, retorna falha)
   */
  override async createPix(_request: PaymentRequest, _config: IntegrationConfig): Promise<PaymentResponse> {
    return { success: false, status: "failed", message: "Carthero não suporta PIX." };
  }
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          success: false,
          status: "failed",
          message: `Carthero rejeitou o cartão: ${body.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
  ...
  /**
   * Processamento de Boleto (Carthero não suporta Boleto, retorna falha)
   */
  override async createBoleto(_request: PaymentRequest, _config: IntegrationConfig): Promise<PaymentResponse> {
    return { success: false, status: "failed", message: "Carthero não suporta boleto." };
  }
  ...
  override async consultPayment(gatewayTransactionId: string, config: IntegrationConfig): Promise<PaymentStatusResponse> {
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(`Erro ao consultar pagamento no Carthero (${res.status}): ${body.message || "Erro desconhecido"}`);
      }
    } catch (err: any) {
