...
export class Mapper {
  /**
   * Converte PaymentRequest no payload da Carthero.
   */
  static toCreatePaymentPayload(request: PaymentRequest, webhookUrl?: string): CreatePaymentPayload {
    ...
    else if (request.paymentMethod === "debit_card") payment_method = "debit_card";

    // Carthero espera amount em centavos (padrão APIs brasileiras)
    ...
    if (request.card && (payment_method === "credit_card" || payment_method === "debit_card")) {
      payload.card = {
  ...
  /**
   * Converte resposta da Carthero para o padrão interno.
   */
  static toPaymentResponse(
    ...
    if (apiResponse.error || (!apiResponse.id && !apiResponse.transaction_id)) {
      return {
        success: false,
        status: "failed",
        message: apiResponse.error?.message || apiResponse.message || "Carthero recusou o pagamento.",
    ...
    const response: InternalPaymentResponse = {
      ...
      status: statusVal,
      message: apiResponse.message || `Carthero status: ${apiResponse.status}`,
      raw: apiResponse,
    };
  ...
  /**
   * Normaliza os códigos de status da Carthero.
   */
