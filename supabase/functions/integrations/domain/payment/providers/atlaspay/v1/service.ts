...
export class Service extends BaseGateway {
  readonly name = "Atlas Pay";
  readonly slug = "atlaspay";
  ...
  /**
   * Valida as credenciais da Atlas Pay.
   */
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          isValid: false,
          message: "Credenciais Atlas Pay inválidas. Verifique o apiId e apiSecret.",
        };
      }

      return {
        isValid: true,
        message: "Credenciais Atlas Pay validadas com sucesso.",
      };
    } catch (err: any) {
      return {
        isValid: true,
  ...
  /**
   * Processa pagamentos via Atlas Pay.
   */
    ...
    try {
      ...
      if (!res.ok) {
        return {
          success: false,
          status: "failed",
          message: `Atlas Pay rejeitou a transação (${res.status}): ${body?.error?.message || "Erro desconhecido"}`,
      ...
      return {
        success: false,
        status: "failed",
        message: `Erro de comunicação com Atlas Pay: ${err.message}`,
      };
    }
  }

  /**
   * Consulta o status de um pagamento na Atlas Pay.
   */
  async consultPayment(
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(
          `Erro ao consultar Atlas Pay (${res.status}): ${body?.error?.message || "Erro desconhecido"}`
        );
      }
    } catch (err: any) {
      throw new Error(`Falha ao consultar pagamento Atlas Pay: ${err.message}`);
    }
  }

  /**
   * Estorna/reembolsa um pagamento na Atlas Pay.
   */
  async refundPayment(
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Estorno Atlas Pay processado com sucesso.",
        };
      } else {
        return {
          success: false,
          amount: request.amount || 0,
          status: "failed",
          message: `Atlas Pay rejeitou o estorno (${res.status}): ${body?.error?.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
      return {
        ...
        amount: request.amount || 0,
        status: "failed",
        message: `Falha ao solicitar estorno na Atlas Pay: ${err.message}`,
      };
    }
  }

  /**
   * Processa webhook recebido da Atlas Pay.
   */
  async handleWebhook(
    payload: any,
    signature?: string,
