...
export class Service extends BaseGateway {
  ...
  /**
   * Valida as credenciais da Bestfy.
   */
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          isValid: false,
          message: "Credenciais Bestfy inválidas. Verifique o token.",
        };
      }

      return {
        isValid: true,
        message: "Credenciais Bestfy validadas com sucesso.",
      };
    } catch (err: any) {
      return {
        isValid: true,
  ...
  /**
   * Processa pagamentos via Bestfy.
   */
    ...
    try {
      ...
      if (!res.ok) {
        return {
          success: false,
          status: "failed",
          message: `Bestfy rejeitou a transação (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
      ...
      return {
        success: false,
        status: "failed",
        message: `Erro de comunicação com Bestfy: ${err.message}`,
      };
    }
  }

  /**
   * Consulta o status de um pagamento na Bestfy.
   */
  async consultPayment(
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(
          `Erro ao consultar Bestfy (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`
        );
      }
    } catch (err: any) {
      throw new Error(`Falha ao consultar pagamento Bestfy: ${err.message}`);
    }
  }

  /**
   * Estorna/reembolsa um pagamento na Bestfy.
   */
  async refundPayment(
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Estorno Bestfy processado com sucesso.",
        };
      } else {
        return {
          success: false,
          amount: request.amount || 0,
          status: "failed",
          message: `Bestfy rejeitou o estorno (${res.status}): ${body?.error?.message || body?.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
      return {
  ...
  /**
   * Processa webhook recebido da Bestfy.
   */
  async handleWebhook(
    payload: any,
    signature?: string,
