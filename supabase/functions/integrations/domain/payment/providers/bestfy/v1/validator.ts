...
export class Validator {
  ...
  static validatePaymentRequest(request: PaymentRequest): { isValid: boolean; errors: string[] } {
    ...
    if (request.paymentMethod === "credit_card") {
      const hasToken = !!request.metadata?.token || !!request.metadata?.hash;
      ...
      if (!hasToken && !hasCardDetails) {
        errors.push("Para pagamento com Cartão de Crédito, envie os detalhes do cartão ou um hash pré-gerado.");
      }
    }
