...
export class Mapper {
  /**
   * Converte PaymentRequest no payload da Axelpay.
   */
  ...
  /**
   * Converte resposta da Axelpay para o padrão interno.
   */
  static toPaymentResponse(
    ...
    if (apiResponse.error || !apiResponse.id) {
      return {
        success: false,
        status: "failed",
        message: apiResponse.error?.message || "Axelpay recusou o pagamento.",
    ...
    const response: InternalPaymentResponse = {
      ...
      message: `Axelpay status: ${apiResponse.status}`,
      raw: apiResponse,
    };
  ...
  /**
   * Normaliza os códigos de status da Axelpay.
   */
