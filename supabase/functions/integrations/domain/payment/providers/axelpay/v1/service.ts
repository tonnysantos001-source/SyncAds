...
export class Service extends BaseGateway {
  readonly name = "Axelpay";
  readonly slug = "axelpay";
  ...
  /**
   * Valida as credenciais da Axelpay.
   */
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          isValid: false,
          message: "Credenciais Axelpay inválidas. Verifique o clientId e clientSecret.",
        };
      }

      return {
        isValid: true,
        message: "Credenciais Axelpay validadas com sucesso.",
      };
    } catch (err: any) {
      return {
        isValid: true,
  ...
  /**
   * Processa pagamentos via Axelpay.
   */
    ...
    try {
      ...
      if (!res.ok) {
        return {
          success: false,
          status: "failed",
          message: `Axelpay rejeitou a transação (${res.status}): ${body?.error?.message || "Erro desconhecido"}`,
      ...
      return {
        success: false,
        status: "failed",
        message: `Erro de comunicação com Axelpay: ${err.message}`,
      };
    }
  }

  /**
   * Consulta o status de um pagamento na Axelpay.
   */
  async consultPayment(
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(
          `Erro ao consultar Axelpay (${res.status}): ${body?.error?.message || "Erro desconhecido"}`
        );
      }
    } catch (err: any) {
      throw new Error(`Falha ao consultar pagamento Axelpay: ${err.message}`);
    }
  }

  /**
   * Estorna/reembolsa um pagamento na Axelpay.
   */
  async refundPayment(
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Estorno Axelpay processado com sucesso.",
        };
      } else {
        return {
          success: false,
          amount: request.amount || 0,
          status: "failed",
          message: `Axelpay rejeitou o estorno (${res.status}): ${body?.error?.message || "Erro desconhecido"}`,
        };
      }
    } catch (err: any) {
      return {
        ...
        amount: request.amount || 0,
        status: "failed",
        message: `Falha ao solicitar estorno na Axelpay: ${err.message}`,
      };
    }
  }

  /**
   * Processa webhook recebido da Axelpay.
   */
  async handleWebhook(
    payload: any,
    signature?: string,
