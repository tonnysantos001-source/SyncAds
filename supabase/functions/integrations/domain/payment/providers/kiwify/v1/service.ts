...
export class Service extends BaseGateway {
  ...
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    return { isValid: true, message: "Credenciais Kiwify aceitas com sucesso." };
  }
  ...
  async processPayment(request: PaymentRequest, config: IntegrationConfig): Promise<PaymentResponse> {
    ...
    try {
      ...
      if (!res.ok) return { success: false, status: "failed", message: body?.message || `Erro Kiwify ${res.status}`, raw: body };
      return Mapper.toPaymentResponse(body, request.orderId);
    } catch (e: any) {
      return { success: false, status: "failed", message: `Erro na comunicacao com Kiwify: ${e.message}` };
    }
  }
  ...
  async consultPayment(gatewayTransactionId: string, config: IntegrationConfig): Promise<PaymentStatusResponse> {
    ...
    const body = await res.json();
    if (!res.ok) throw new Error(`Erro ao consultar Kiwify: ${res.status}`);
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Estorno Kiwify realizado."
        };
      }
      return {
        success: false,
        amount: request.amount || 0,
        status: "failed",
        message: body?.message || `Erro estorno Kiwify: ${res.status}`
      };
    } catch (e: any) {
