...
export class Service extends BaseGateway {
  ...
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    return v.isValid ? { isValid: true, message: "Credenciais Zoop válidas." } : { isValid: false, message: v.errors.join(", ") };
  }
  async processPayment(request: PaymentRequest, config: IntegrationConfig): Promise<PaymentResponse> {
    ...
    try {
      const res = await this.getClient(config).createTransaction(payload);
      const body = await res.json();
      if (!res.ok) return { success: false, status: "failed", message: body?.error?.message || `Zoop ${res.status}`, raw: body };
      return Mapper.toPaymentResponse(body, request.orderId);
    } catch (e: any) { return { success: false, status: "failed", message: `Erro Zoop: ${e.message}` }; }
  }
  async consultPayment(gatewayTransactionId: string, config: IntegrationConfig): Promise<PaymentStatusResponse> {
    ...
    const body = await res.json();
    if (!res.ok) throw new Error(`Zoop ${res.status}`);
  ...
  async refundPayment(request: RefundRequest, config: IntegrationConfig): Promise<RefundResponse> {
    try {
      const res = await this.getClient(config).voidTransaction(request.gatewayTransactionId);
      const body = await res.json().catch(() => ({}));
      return res.ok ? { success: true, refundId: request.gatewayTransactionId, amount: request.amount || 0, status: "approved", message: "Zoop void processado." } : { success: false, amount: request.amount || 0, status: "failed", message: body?.error?.message || "Zoop void negado." };
