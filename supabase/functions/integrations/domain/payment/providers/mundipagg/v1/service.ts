...
export class Service extends BaseGateway {
  readonly name = "MundiPagg";
  readonly slug = "mundipagg";
  ...
  async validateCredentials(credentials: any): Promise<CredentialValidationResult> {
    ...
    return { isValid: true, message: "Credenciais MundiPagg aceitas com sucesso." };
  }
  ...
  async processPayment(request: PaymentRequest, config: IntegrationConfig): Promise<PaymentResponse> {
    ...
    try {
      ...
      if (!res.ok) return { success: false, status: "failed", message: body?.message || `Erro MundiPagg ${res.status}`, raw: body };
      return Mapper.toPaymentResponse(body, request.orderId);
    } catch (e: any) {
      return { success: false, status: "failed", message: `Erro na comunicacao com MundiPagg: ${e.message}` };
    }
  }
  ...
  async consultPayment(gatewayTransactionId: string, config: IntegrationConfig): Promise<PaymentStatusResponse> {
    ...
    const body = await res.json();
    if (!res.ok) throw new Error(`Erro ao consultar MundiPagg: ${res.status}`);
    ...
    try {
      ...
      if (res.ok) {
        return {
          ...
          status: "approved",
          message: "Estorno MundiPagg realizado."
        };
      }
      return {
        success: false,
        amount: request.amount || 0,
        status: "failed",
        message: body?.message || `Erro estorno MundiPagg: ${res.status}`
      };
    } catch (e: any) {
