...
export class Validator {
  static validateCredentials(credentials: any): { isValid: boolean; errors: string[] } {
    ...
    if (!credentials.developerKey) errors.push("developerKey is required");
    if (!credentials.clientId) errors.push("clientId is required");
    if (!credentials.clientSecret) errors.push("clientSecret is required");
