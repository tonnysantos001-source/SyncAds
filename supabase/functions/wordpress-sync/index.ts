...
serve(async (req) => {
  ...
  try {
    ...
    const { data: integration } = await supabase.from("WordpressIntegration").select("*").eq("id", integrationId).single();
    ...
    const items: any[] = [];
    await supabase.from("WordpressIntegration").update({ lastSyncAt: new Date().toISOString(), lastSyncStatus: "success" }).eq("id", integration.id);
