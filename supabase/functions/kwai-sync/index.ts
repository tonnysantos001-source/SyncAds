...
serve(async (req) => {
  ...
  try {
    ...
    const { data: integration } = await supabase.from("KwaiIntegration").select("*").eq("id", integrationId).single();
    ...
    const items: any[] = [];
    await supabase.from("KwaiIntegration").update({ lastSyncAt: new Date().toISOString(), lastSyncStatus: "success" }).eq("id", integration.id);
