...
serve(async (req) => {
  ...
  try {
    ...
    const { data: integration } = await supabase.from("AhrefsIntegration").select("*").eq("id", integrationId).single();
    ...
    const items: any[] = [];
    await supabase.from("AhrefsIntegration").update({ lastSyncAt: new Date().toISOString(), lastSyncStatus: "success" }).eq("id", integration.id);
