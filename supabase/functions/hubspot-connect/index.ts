...
serve(async (req) => {
  ...
  try {
    ...
    const integrationId = `${user.id}-Hubspot`;
    await supabase.from("HubspotIntegration").upsert({ id: integrationId, userId: user.id, apiKey, credentials, isActive: true, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }, { onConflict: "userId" });
